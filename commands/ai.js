// ========== ai.js - UNIVERSAL PRECISE RESPONSES ==========
...
module.exports = {
  ...
  description: 'Complete AI assistant with precise organized responses',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== DETECT LANGUAGE, INTENT, TOPIC, SUBJECT =====
      ...
      const finalPrompt = this.buildPrecisePrompt(prompt, previousResponse, previousPrompt, language, intent, topic, subject, requestType);
  ...
  detectSubject(prompt) {
    ...
    if (/survey|interview|observation|questionnaire|research instrument|data collection|methodology|respondent|sampling|focus group|case study|experiment|ethnography|phenomenology/i.test(lower)) {
      return 'research_tools';
    }
    if (/criticize|critique|observe|observation|analyze|analysis|review|evaluate|assess|comment|react|reflection/i.test(lower)) {
      return 'critique';
    }
    if (/research|thesis|study|dissertation|title|proposal/i.test(lower)) {
      return 'research';
    }
    if (/photosynthesis|cell|dna|protein|enzyme|organism|ecosystem|evolution|genetics|biology|chemistry|physics|atom|molecule|compound|chemical|reaction|gravity|energy/i.test(lower)) {
      return 'science';
    }
    if (/solve|equation|formula|calculate|compute|x =|algebra|geometry|trigonometry|calculus/i.test(lower)) {
      return 'math';
    }
    if (/history|rizal|bonifacio|lapu-lapu|revolution|world war|kasaysayan|president|hero|bayani/i.test(lower)) {
      return 'history';
    }
    if (/noun|verb|adjective|grammar|sentence|paragraph|essay|literature|poem|novel|story/i.test(lower)) {
      return 'english';
    }
    if (/panghalip|pangngalan|pandiwa|pang-uri|pang-abay|tula|sanaysay|kwento|balarila/i.test(lower)) {
      return 'filipino';
    }
    if (/python|java|javascript|c\+\+|programming|code|function|variable|loop|array/i.test(lower)) {
      return 'coding';
    }
    if (/health|disease|symptom|treatment|medicine|doctor|nurse|nutrition|exercise/i.test(lower)) {
      return 'health';
    }
    if (/business|management|marketing|finance|accounting|entrepreneur|economics/i.test(lower)) {
      return 'business';
    }
    if (/logic|puzzle|riddle|reasoning/i.test(lower)) {
      return 'logic';
    }
  ...
  detectIntent(prompt) {
    ...
    if (/survey|interview|observation|questionnaire|research instrument|data collection/i.test(lower)) return 'research_tool';
    if (/difference|compare|contrast|vs|versus|kaibahan|pagkakaiba/i.test(lower)) return 'comparison';
    if (/criticize|critique|observe|observation|analyze|analysis|review|evaluate|assess|comment on|react to|reflection/i.test(lower)) return 'critique';
  ...
  // ============================================================
  // BUILD PRECISE PROMPT - Universal
  // ============================================================
  buildPrecisePrompt(prompt, previousResponse, previousPrompt, language, intent, topic, subject, requestType) {
    ...
    if (requestType === 'summarize') {
      finalPrompt += `Give a brief summary (3-5 sentences) about "${topic}" in ${langName}.\n`;
      finalPrompt += `Be balanced and accurate.\n\n`;
    } else if (requestType === 'short') {
      finalPrompt += `Give a short, direct answer (1-3 sentences) about "${topic}" in ${langName}.\n\n`;
    } else if (requestType === 'translate') {
      finalPrompt += `Translate accurately.\n\n`;
    } else if (requestType === 'detailed') {

      // ===== RESEARCH TOOLS (Survey, Interview, Observation, etc.) =====
      if (subject === 'research_tools' || intent === 'research_tool') {
        finalPrompt += `Explain "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `DEFINITION:\n[Clear definition of "${topic}"]\n\n`;
        finalPrompt += `PURPOSE:\n[What is it used for]\n\n`;
        finalPrompt += `KEY CHARACTERISTICS:\n1. [Characteristic]\n2. [Characteristic]\n3. [Characteristic]\n\n`;
        finalPrompt += `TYPES:\n1. [Type]\n2. [Type]\n3. [Type]\n\n`;
        finalPrompt += `ADVANTAGES:\n1. [Advantage]\n2. [Advantage]\n\n`;
        finalPrompt += `DISADVANTAGES:\n1. [Disadvantage]\n2. [Disadvantage]\n\n`;
        finalPrompt += `EXAMPLES:\n1. [Example]\n2. [Example]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }

      // ===== COMPARISON =====
      else if (intent === 'comparison') {
        finalPrompt += `Compare "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `DEFINITION OF EACH:\n1. [First item]\n2. [Second item]\n3. [Third item if applicable]\n\n`;
        finalPrompt += `SIMILARITIES:\n1. [Similarity]\n2. [Similarity]\n3. [Similarity]\n\n`;
        finalPrompt += `DIFFERENCES:\n1. [Difference]\n2. [Difference]\n3. [Difference]\n\n`;
        finalPrompt += `COMPARISON TABLE:\n[Aspect] | [Item 1] | [Item 2]\n`;
        finalPrompt += `[Row 1]\n[Row 2]\n[Row 3]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }
      ...
      // ===== CRITIQUE =====
        ...
        finalPrompt += `Analyze "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[2-3 sentences about "${topic}"]\n\n`;
        finalPrompt += `MAIN POINTS:\n1. [First important point]\n2. [Second important point]\n3. [Third important point]\n\n`;
        finalPrompt += `ANALYSIS:\n[Balanced analysis — positive, negative, neutral]\n\n`;
        finalPrompt += `EXAMPLES:\n1. [Specific example]\n2. [Specific example]\n\n`;
        finalPrompt += `RECOMMENDATIONS:\n1. [Specific recommendation]\n2. [Specific recommendation]\n\n`;
        finalPrompt += `CONCLUSION:\n[Balanced summary]\n\n`;
      }
      ...
      // ===== DEFINITION =====
        ...
        finalPrompt += `Define and explain "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `DEFINITION:\n[Clear definition]\n\n`;
        finalPrompt += `KEY CHARACTERISTICS:\n1. [Characteristic]\n2. [Characteristic]\n3. [Characteristic]\n\n`;
        finalPrompt += `EXAMPLES:\n1. [Example]\n2. [Example]\n\n`;
        finalPrompt += `IMPORTANCE:\n[Why it matters]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }

      // ===== PROCESS =====
      else if (intent === 'process') {
        finalPrompt += `Explain the process of "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `DEFINITION:\n[Brief definition]\n\n`;
        finalPrompt += `FULL PROCESS:\n`;
        finalPrompt += `1. [Stage Name]\nDescription: [what happens]\nExample: [specific example]\n\n`;
        finalPrompt += `2. [Stage Name]\nDescription: [what happens]\nExample: [specific example]\n\n`;
        finalPrompt += `...\n\n`;
        finalPrompt += `SUMMARY:\n[Summary]\n\n`;
      }
      ...
      // ===== BIOGRAPHY =====
        ...
        finalPrompt += `Give a complete biography of "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `BACKGROUND:\nFull Name: [name]\nBirth Date: [date]\nBirth Place: [place]\nDeath Date: [date]\n\n`;
        finalPrompt += `EARLY LIFE:\n[Details]\n\n`;
        finalPrompt += `MAJOR CONTRIBUTIONS:\n1. [Contribution]\n2. [Contribution]\n3. [Contribution]\n\n`;
        finalPrompt += `SIGNIFICANCE:\n[Why important]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }

      // ===== LIST =====
      else if (intent === 'list') {
        finalPrompt += `List and explain "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `LIST:\n1. [Item]\nDescription: [details]\nExample: [example]\n\n`;
        finalPrompt += `2. [Item]\nDescription: [details]\nExample: [example]\n\n`;
      ...
      // ===== EXPLANATION =====
        ...
        finalPrompt += `Explain "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `DIRECT ANSWER:\n[Answer to why]\n\n`;
        finalPrompt += `MAIN REASONS:\n1. [Reason]\nExplanation: [details]\nExample: [example]\n\n`;
        finalPrompt += `2. [Reason]\nExplanation: [details]\nExample: [example]\n\n`;
        finalPrompt += `...\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }
      ...
      // ===== HOW TO =====
        ...
        finalPrompt += `Explain how to do "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `STEP-BY-STEP GUIDE:\n`;
        finalPrompt += `Step 1: [Action]\nDetails: [how to do it]\nTip: [useful tip]\n\n`;
        finalPrompt += `Step 2: [Action]\nDetails: [how to do it]\nTip: [useful tip]\n\n`;
        finalPrompt += `...\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }
      ...
      // ===== PROS AND CONS =====
        ...
        finalPrompt += `Analyze the advantages and disadvantages of "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `ADVANTAGES:\n1. [Advantage]\nExplanation: [details]\n\n`;
        finalPrompt += `2. [Advantage]\nExplanation: [details]\n\n`;
        finalPrompt += `DISADVANTAGES:\n1. [Disadvantage]\nExplanation: [details]\n\n`;
        finalPrompt += `2. [Disadvantage]\nExplanation: [details]\n\n`;
        finalPrompt += `BALANCED VIEW:\n[Neutral perspective]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }

      // ===== IMPORTANCE =====
      else if (intent === 'importance') {
        finalPrompt += `Explain the importance of "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `DIRECT ANSWER:\n[Statement of importance]\n\n`;
        finalPrompt += `WHY IT MATTERS:\n1. [Reason]\nExplanation: [details]\n\n`;
        finalPrompt += `2. [Reason]\nExplanation: [details]\n\n`;
        finalPrompt += `IMPACT:\n[Overall impact]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }
      ...
      // ===== EFFECTS =====
        ...
        finalPrompt += `Explain the effects of "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[Brief intro]\n\n`;
        finalPrompt += `POSITIVE EFFECTS:\n1. [Effect]\nExplanation: [details]\n\n`;
        finalPrompt += `2. [Effect]\nExplanation: [details]\n\n`;
        finalPrompt += `NEGATIVE EFFECTS:\n1. [Effect]\nExplanation: [details]\n\n`;
        finalPrompt += `2. [Effect]\nExplanation: [details]\n\n`;
        finalPrompt += `BALANCED VIEW:\n[Neutral perspective]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary]\n\n`;
      }
      ...
      // ===== ESSAY =====
        ...
        finalPrompt += `Write an essay about "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[2-3 sentences introducing "${topic}"]\n\n`;
        finalPrompt += `BODY:\n`;
        finalPrompt += `Paragraph 1: [First main point with examples]\n\n`;
        finalPrompt += `Paragraph 2: [Second main point with examples]\n\n`;
        finalPrompt += `Paragraph 3: [Third main point with examples]\n\n`;
        finalPrompt += `CONCLUSION:\n[Summary and final thoughts]\n\n`;
      }

      // ===== GENERAL =====
      else {
        finalPrompt += `Answer about "${topic}" in ${langName}.\n\n`;
        finalPrompt += `FORMAT:\n`;
        finalPrompt += `INTRODUCTION:\n[2-3 sentences about "${topic}"]\n\n`;
        finalPrompt += `MAIN POINTS:\n1. [Important point]\n2. [Important point]\n3. [Important point]\n\n`;
        finalPrompt += `DETAILS:\n[Specific details]\n\n`;
      ...
      finalPrompt += `Answer about "${topic}" accurately in ${langName}.\n`;
      finalPrompt += `Be balanced and include important details.\n\n`;
    }
    ...
    finalPrompt += `3. Start directly with the content.\n`;
    finalPrompt += `4. Be BALANCED — include different perspectives.\n`;
    finalPrompt += `5. Be SPECIFIC to "${topic}".\n`;
    ...
    finalPrompt += `9. NO "As DeepSeek" or "As an AI" intro.\n`;
    finalPrompt += `10. Be clear, organized, and easy to understand.\n`;
    finalPrompt += `11. Complete the entire response — do not cut off mid-sentence.\n\n`;
  ...
  // ============================================================
  // SEND COMPLETE - Split but continuous
  // ============================================================
