// ========== ai.js - COMPLETE AI ASSISTANT v34.0.1 ==========
// HYBRID FALLBACK: Return agad kapag >=85, subukan pa kapag 60-84
...
module.exports = {
  ...
  description: 'Complete AI assistant with hybrid fallback',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (isFollowUpCommand) {
        ...
        const response = await this.callAPIWithRetry(followUpPrompt, prompt, 'english', action, action, 'followup', 'detailed');
      ...
      const response = await this.callAPIWithRetry(finalPrompt, prompt, language, intent, topic, subject, requestType);
  ...
  // CALL API WITH RETRY
  // ============================================================
  async callAPIWithRetry(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    let response = null;
    let attempts = 0;
    const maxAttempts = 2;
    let lastError = null;
    ...
    while (attempts < maxAttempts && !response) {
      attempts++;
      try {
        response = await this.callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType);
      } catch (error) {
        lastError = error;
        console.log('[AI] Attempt ' + attempts + ' failed: ' + error.message);
  ...
  async callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    const apiConfigs = [
      ...
      { name: 'Pollinations AI', url: 'https://api-library-kohi-production.up.railway.app/api/pollination-ai', param: 'prompt', responsePath: 'data', successField: 'status', timeout: 90000 },
    ...
    for (const config of apiConfigs) {
      try {
        ...
        if (errorKeywords.some(kw => dataStr.includes(kw))) {
          failedApis.push(config.name + ' (temporarily unavailable)');
          console.log('[API] ' + config.name + ' — temporarily unavailable, skipping');
          continue;
        }

        // FIX: Only validate when successValue is explicitly defined
        if (config.successField && config.successValue !== undefined) {
          if (data[config.successField] !== config.successValue) {
            failedApis.push(config.name + ' (invalid field)');
            continue;
          }
        }
        ...
        if (extracted && typeof extracted === 'string' && extracted.trim().length > 0) {
          ...
          if (errorKeywords.some(kw => extractedLower.includes(kw))) {
            failedApis.push(config.name + ' (extracted error)');
            console.log('[API] ' + config.name + ' — extracted text is error, skipping');
            continue;
          }
          ...
          workingApis.push(config.name + ' (' + quality.score + ')');

          // HYBRID LOGIC
          // 1. If EXCELLENT (>=85), return immediately
          if (quality.score >= 85) {
            console.log('[API] EXCELLENT response from ' + config.name + ' (' + quality.score + ') — returning immediately');
            return formatted;
          }

          // 2. If GOOD (60-84), save as best candidate
          if (quality.score > bestScore) {
            bestScore = quality.score;
            bestResponse = formatted;
            bestApiName = config.name;
          }
        } else {
          failedApis.push(config.name + ' (empty)');
        }
      } catch (error) {
        let reason = error.message;
        if (error.response?.data) {
          const errData = JSON.stringify(error.response.data).toLowerCase();
          if (errorKeywords.some(kw => errData.includes(kw))) {
            reason = 'temporarily unavailable';
          }
        }
        failedApis.push(config.name + ' (' + reason + ')');
        console.log('[API] ' + config.name + ' failed: ' + reason);
      }
    }

    console.log('[API] Working: ' + (workingApis.length > 0 ? workingApis.join(', ') : 'NONE'));
    console.log('[API] Failed: ' + (failedApis.length > 0 ? failedApis.join(', ') : 'NONE'));

    if (bestResponse) {
      console.log('[API] BEST available: ' + bestApiName + ' (score: ' + bestScore + ')');
      return bestResponse;
    }

    throw new Error('All APIs failed. Details: ' + failedApis.join(' | '));
  },
  ...
  async handleTranslation(senderId, prompt, token) {
    try {
      ...
      for (const [key, value] of Object.entries(languageMap)) {
        // FIX: word boundary to avoid "sa" matching "isalin"
        if (new RegExp('\\b' + key + '\\b', 'i').test(lower)) { targetLang = value; break; }
      }
      ...
      const response = await this.callAPIWithRetry(finalPrompt, prompt, 'english', 'translation', 'translation', 'translation', 'general');
      let cleaned = this.finalClean(response);
  ...
  async handleWordList(senderId, prompt, token) {
    try {
      ...
      finalPrompt += '10. Include: synonyms, antonyms, related terms, formal, casual, Filipino, Bisaya.\n';
      finalPrompt += '11. NO emojis, NO markdown.\n\n';
      ...
      const response = await this.callAPIWithRetry(finalPrompt, prompt, 'english', 'wordlist', 'wordlist', 'wordlist', 'general');
      let cleaned = this.finalClean(response);
  ...
  isFollowUpCommand(cleanPrompt) {
    ...
    // FIX: require follow-up to be a short command, not a new question containing "short"
    if (p.length > 60) return false;
    ...
    if (/^(simplify|i-simplify|pasimplehin|gawing simple|gawing madali|madaliin|simple version|easy version|easy to understand|madaling maintindihan)( it| this| that| mo| po| lang| naman)?$/i.test(p)) return true;
  ...
  getFollowUpAction(cleanPrompt) {
    const p = cleanPrompt.toLowerCase().trim();
    if (/summar|buod|sum up|shorten|paikli|iklian|make it short|^short$|^brief$|^concise$/i.test(p)) return 'summarize';
  ...
  buildFollowUpPrompt(action, previousResponse, userCommand) {
    const actionInstructions = {
      ...
      'summarize': 'SUMMARIZE the following response. Make it SHORT and CONCISE. Keep ONLY the key points. Use a MAXIMUM of 200 words. Remove all unnecessary details.',
  ...
  async handleWeightEstimation(senderId, prompt, token) {
    try {
      ...
      // FIX: extract via labeled patterns FIRST, then fallback to positional
      ...
      // FIX: only fall back to positional if BOTH girth and length are still null
      const allNumbers = (prompt.match(/\d+\.?\d*/g) || []).map(Number);
      if (girth === null && length === null && allNumbers.length >= 2) {
        girth = allNumbers[0];
        length = allNumbers[1];
      } else if (girth === null && allNumbers.length >= 1 && !volumeMatch) {
        // single-number fallback only for chicken/duck/turkey (girth-only formulas)
        girth = allNumbers[0];
      }
      if (volume === null && allNumbers.length >= 1 && !girthMatch && !lengthMatch) {
        volume = allNumbers[0];
      }
  ...
  buildWoodFormula(allNumbers, lower) {
    ...
    const wl = allNumbers[0];
    const width = allNumbers[1];
    const thickness = allNumbers[2];
    let density = 0.55;
  ...
  buildGrainFormula(volume, allNumbers, lower) {
    const liters = volume !== null ? volume : (allNumbers.length >= 1 ? allNumbers[0] : null);
    if (liters === null) {
    ...
    try {
      ...
      if (familyMatch) {
        ...
        if (/each (son|daughter|brother|sister|child) has a (sister|brother)/i.test(prompt)) {
          // FIX: siblings is unused; calculate totalChildren directly
          let totalChildren = count;
          ...
          if (hasSharedSister && !/daughters?/i.test(type)) totalChildren += 1;
          if (hasSharedBrother && !/sons?/i.test(type)) totalChildren += 1;
      ...
      const response = await this.callAPIWithRetry(logicPrompt, prompt, 'english', 'logic', 'logic', 'logic', 'general');
      let cleaned = this.finalClean(response);
  ...
  validateResponseQuality(response, intent) {
    ...
    // FIX: isGood now matches hybrid threshold (>=60 is acceptable; >=85 is excellent)
    return { isGood: score >= 85, isAcceptable: score >= 60, score: Math.max(0, score) };
  },
  ...
  detectSubject(prompt) {
    ...
    // FIX: correct ordering — "process of the cycle" and "cycle process"
  ...
  buildBriefPrompt(prompt, langName, intent) {
    ...
    finalPrompt += '9. NO emojis, NO markdown, NO LaTeX.\n';
    finalPrompt += '10. Respond in ' + langName.toUpperCase() + ' language.\n\n';
  ...
  splitComplete(text) {
    ...
    while (remaining.length > 0) {
      ...
      // FIX: clamp to MAX_CHUNK to avoid exceeding limit when bp.char.length > 1
  ...
  tryMathCompute(prompt) {
    ...
    const lower = prompt.toLowerCase();
    const nonMathKeywords = ['summarize', 'summary', 'buod', 'shorten', 'paikliin', 'brief', 'concise', 'short', 'maikli', 'translate', 'isalin', 'salin', 'explain', 'paliwanag', 'ipaliwanag', 'detail', 'elaborate', 'what', 'ano', 'why', 'bakit', 'how', 'paano', 'who', 'sino', 'when', 'kailan', 'where', 'saan', 'which', 'alin', 'describe', 'ilarawan', 'give', 'bigay', 'list', 'enumerate', 'tell', 'sabi', 'write', 'isulat', 'make', 'gawa', 'help', 'tulong', 'about', 'tungkol', 'define', 'process', 'steps', 'full', 'complete', 'examples', 'criticize', 'critique', 'observe', 'observation', 'analyze', 'analysis', 'survey', 'interview', 'elaborate', 'difference', 'compare', 'riddle', 'bugtong', 'puzzle', 'logic', 'qualitative', 'quantitative', 'thesis', 'experimental', 'research', 'weight', 'timbang', 'girth', 'length', 'synonym', 'antonym', 'other term', 'another word', 'kasingkahulugan', 'kasalungat'];
  ...
  buildMathSteps(clean) {
    const steps = [];
    // FIX: check stripped version for addition-only / subtraction-only
    const stripped = clean.replace(/\s/g, '');
    if (/^[\d\.\+]+$/.test(stripped)) {
      const numbers = stripped.match(/\d+\.?\d*/g).map(Number);
      let current = 0;
  ...
  getErrorMessage(error) {
    ...
    if (error.response?.status === 402) return 'AI service limit reached. Please try again later.';
    if (error.response?.status >= 500) return 'Server error. Please try again later.';
    const errMsg = (error.message || '').toLowerCase();
    if (errMsg.includes('credits') || errMsg.includes('balance') || errMsg.includes('top up')) {
