...
// Full Math Support: Algebra, Linear, Quadratic, Percentage, Fractions,
// Statistics, Geometry, Trigonometry, Arithmetic, Calculus, Physics,
// Chemistry, Finance, CS, and 60+ more handlers
...
module.exports = {
  ...
  description: 'Complete AI assistant with full math support',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Specific first, generic last. No duplication.
      // ============================================================
      const HANDLER_ORDER = [
        // --- Word problems & measurement (specific phrasing) ---
        'tryWordProblem',
        'tryUnitConversion',
        'tryMeasurement',
        'tryUnitPrefix',
        ...
        // --- Statistics family (specific → general) ---
        'tryDistribution',
        'tryRegression',
        'tryCorrelation',
        'tryNormalDist',
        ...
        // --- CS / Logic family (specific → general) ---
        'tryBoolean',
        'tryLogicGates',
        'tryBinaryHex',
        'tryBaseConversion',
      ...
      // ============================================================
      // STEP 6: LOGIC (uses API, keep separate)
      // ============================================================
...
expandBinomial(a, b) {
  ...
  const combineSteps = combined.map(t => {
    return '  ' + t.raw + ' (degree ' + t.degree + ')';
  }).join('\n');
...
parseTerms(expr) {
  ...
  for (let i = 0; i < cleaned.length; i++) {
    ...
    if ((c === '+' || c === '-') && i > 0) {
      if (current) {
        terms.push(this.buildTerm(current, sign));
        current = '';
      }
      sign = c === '-' ? -1 : 1;
    } else if (i === 0 && c === '-') {
...
multiplyTerms(t1, t2) {
  ...
  for (const [v, e] of Object.entries(t2.vars)) {
    vars[v] = (vars[v] || 0) + e;
  }
  ...
  const sign = coeff < 0 ? '-' : '';
  const raw = sign + coeffStr + varStr;
...
// ============================================================
// HANDLER 2: LINEAR EQUATION (FIXED: variable on both sides)
// ============================================================
tryLinearEquation(prompt) {
  if (!prompt) return null;
  const lower = prompt.toLowerCase();

  if (!lower.includes('solve')) return null;
  if (!/[a-z]/i.test(prompt)) return null;
  if (!/=/.test(prompt)) return null;
  if (/x\^?2/i.test(prompt)) return null;

  const eqMatch = prompt.match(/solve\s*:?\s*([^=]+=\s*[^=\s]+)/i);
  ...
  try {
    ...
    // Parse each side into ax + b
    const parseSide = (side, varName) => {
      // Match terms: coefficient*variable and constant
      let coeff = 0;
      let constant = 0;

      // Match variable term (with optional coefficient)
      ...
      if (constTermMatch) {
        // Skip if it's the same as the var coefficient
        if (!varTermMatch || constTermMatch.index !== varTermMatch.index) {
          constant = parseFloat(constTermMatch[1]);
        }
      }

      // Handle case where constant comes before variable (e.g., "5 + 2x")
      const reversedMatch = side.match(/([\-\+]?\d+\.?\d*)\s*([\-\+]?\d*\.?\d*)\s*([a-z])/i);
      if (reversedMatch && !constTermMatch) {
        constant = parseFloat(reversedMatch[1]);
      }
    ...
    const L = parseSide(lhs, varName);
    const R = parseSide(rhs, varName);

    // Move all to left: (L.coeff - R.coeff)x + (L.constant - R.constant) = 0
...
// ============================================================
// HANDLER 3: QUADRATIC EQUATION (FIXED: cMatch regex)
// ============================================================
tryQuadraticEquation(prompt) {
  ...
  try {
    ...
    // FIXED: Better regex for coefficient extraction
    let a = 1, b = 0, c = 0;
    ...
    // Match c (constant — after removing x^2 and x terms)
...
tryFractions(prompt) {
  ...
  try {
    ...
    if (op === '+' || op === '-') {
      ...
      if (op === '+') {
        ...
        steps = [
          ...
          'Step 1: Find common denominator: ' + d1 + ' x ' + d2 + ' = ' + commonD,
          'Step 2: Convert fractions:',
          '        ' + n1 + '/' + d1 + ' = ' + newN1 + '/' + commonD,
          '        ' + n2 + '/' + d2 + ' = ' + newN2 + '/' + commonD,
          'Step 3: Add numerators: ' + newN1 + ' + ' + newN2 + ' = ' + resultN,
        ...
        steps = [
          ...
          '        ' + n1 + '/' + d1 + ' = ' + newN1 + '/' + commonD,
          '        ' + n2 + '/' + d2 + ' = ' + newN2 + '/' + commonD,
          'Step 3: Subtract numerators: ' + newN1 + ' - ' + newN2 + ' = ' + resultN,
          'Step 4: Result: ' + resultN + '/' + commonD
        ];
      }
      resultD = commonD;
    ...
    response += 'III. FINAL ANSWER\n';
    response += '   ' + simplifiedN + '/' + simplifiedD;
    return response;
  } catch (e) {
    return null;
  }
...
// ============================================================
// HANDLER 6: STATISTICS (FIXED: number extraction)
// ============================================================
...
// HELPER: Extract numbers from prompt (avoids dates/IDs)
extractNumbers(prompt) {
  if (!prompt) return null;
  // Find number clusters (after 'of', ':', or standalone sequences)
...
// ============================================================
// HANDLER 7: GEOMETRY (Area, Perimeter, Circumference)
// ============================================================
tryGeometry(prompt) {
  if (!prompt) return null;
...
// ============================================================
// HANDLER 8: TRIGONOMETRY (sin, cos, tan)
// ============================================================
tryTrigonometry(prompt) {
  if (!prompt) return null;

  const match = prompt.match(/(sin|cos|tan)\s*\(?\s*([\d\.]+)\s*(?:°|degrees?|deg)?\s*\)?/i);
  if (!match) return null;

  try {
    const func = match[1].toLowerCase();
    const angle = parseFloat(match[2]);
    const isDegree = /°|degree|deg/i.test(prompt) || angle <= 360;
...
// ============================================================
// HANDLER 9: ARITHMETIC (Basic Math with MDAS)
// ============================================================
...
// FIXED: buildMDASSteps with parentheses support
buildMDASSteps(expression) {
  const steps = [];
  let expr = expression.replace(/\s+/g, '');
  steps.push('Original: ' + expr);

  // If has parentheses, resolve innermost first
  while (/\(/.test(expr)) {
...
tryWordProblem(prompt) {
  ...
  if (distanceMatch) {
    ...
    response += 'Problem: ' + prompt + '\n\n';
    response += 'I. GIVEN\n';
    response += '   Speed = ' + speed + ' ' + unit + '\n';
    response += '   Time = ' + time + ' hours\n\n';
    response += 'II. FORMULA\n';
    response += '   Distance = Speed x Time\n\n';
  ...
  if (workMatch) {
    ...
    response += 'Problem: ' + prompt + '\n\n';
    response += 'I. GIVEN\n';
    response += '   Workers 1 = ' + w1 + '\n';
    response += '   Days 1 = ' + d1 + '\n';
    response += '   Workers 2 = ' + w2 + '\n\n';
    response += 'II. FORMULA (Inverse Proportion)\n';
    response += '   W1 x D1 = W2 x D2\n\n';
    response += 'III. COMPUTATION\n';
    response += '   ' + w1 + ' x ' + d1 + ' = ' + w2 + ' x D2\n';
    response += '   ' + (w1 * d1) + ' = ' + w2 + ' x D2\n';
    response += '   D2 = ' + (w1 * d1) + ' / ' + w2 + '\n';
    response += '   D2 = ' + d2.toFixed(2) + ' days\n\n';
    response += 'IV. FINAL ANSWER\n';
    response += '   ' + d2.toFixed(2) + ' days';
    return response;
  }

  // Percentage word problem
  const percentWordMatch = prompt.match(/what\s+is\s+([\d\.]+)\s*%\s*of\s+([\d\.]+)/i);
  if (percentWordMatch) {
    const percent = parseFloat(percentWordMatch[1]);
    const number = parseFloat(percentWordMatch[2]);
    const result = (percent / 100) * number;
    let response = 'WORD PROBLEM SOLUTION\n\n';
    response += 'Problem: ' + prompt + '\n\n';
  ...
  if (ageMatch) {
    ...
    response += '   ' + name2 + ' = x\n';
    response += '   ' + name1 + ' = x + ' + diff + '\n\n';
    response += 'II. EQUATION\n';
    response += '   x + (x + ' + diff + ') = ' + sum + '\n\n';
    response += 'III. SOLVE\n';
    response += '   2x + ' + diff + ' = ' + sum + '\n';
    response += '   2x = ' + (sum - diff) + '\n';
    response += '   x = ' + age2 + '\n\n';
    response += 'IV. FINAL ANSWER\n';
    response += '   ' + name2 + ' = ' + age2 + ' years old\n';
    response += '   ' + name1 + ' = ' + age1 + ' years old';
    return response;
  }
...
// ============================================================
// HANDLER 12: INEQUALITIES
// ============================================================
tryInequality(prompt) {
  if (!prompt) return null;
  const lower = prompt.toLowerCase();

  if (!/solve/i.test(lower)) return null;
  if (!/[<>]/.test(prompt)) return null;
  if (!/[a-z]/i.test(prompt)) return null;

  const match = prompt.match(/solve\s*:?\s*([^<>]+)\s*([<>]=?)\s*([^<>]+)/i);
  if (!match) return null;
  ...
  try {
    ...
    response += '2. Move constant to right side\n';
    response += '   ' + a + varName + ' ' + op + ' ' + c + (b >= 0 ? ' - ' + b : ' + ' + Math.abs(b)) + '\n';
    response += '   ' + a + varName + ' ' + op + ' ' + (c - b) + '\n';
    response += '3. Divide both sides by ' + a + '\n';
    if (a < 0) response += '   (Flip operator since dividing by negative)\n';
    response += '   ' + varName + ' ' + finalOp + ' ' + solution + '\n\n';
    response += 'II. VERIFICATION\n';
    const testVal = finalOp.includes('>') ? solution + 1 : solution - 1;
    response += '   Test ' + varName + ' = ' + testVal + ':\n';
    response += '   ' + a + '(' + testVal + ')' + (b >= 0 ? ' + ' + b : ' - ' + Math.abs(b)) + ' = ' + (a * testVal + b) + '\n';
    response += '   ' + (a * testVal + b) + ' ' + op + ' ' + c + ' - ';
    response += (op.includes('>') ? (a * testVal + b > c) : (a * testVal + b < c)) ? 'TRUE' : 'FALSE';
    response += '\n\n';
    response += 'III. FINAL ANSWER\n';
    response += '   ' + varName + ' ' + finalOp + ' ' + solution;
    return response;
...
tryDerivative(prompt) {
  ...
  try {
    ...
    if (derived.length === 0) {
      derived.push({ coeff: 0, vars: {}, raw: '0', degree: 0 });
    }
...
tryLogarithm(prompt) {
  ...
  const logMatch = prompt.match(/log\s*(?:base\s*)?(\d+)?\s*(?:of\s*)?([\d\.]+)/i);
  if (logMatch) {
    ...
    response += '   log_b(x) = y means b^y = x\n\n';
    response += 'II. COMPUTATION\n';
    ...
    response += 'IV. FINAL ANSWER\n';
    response += '   log_' + base + '(' + value + ') = ' + result.toFixed(4);
    return response;
  }

  const lnMatch = prompt.match(/ln\s*\(?\s*([\d\.]+)\s*\)?/i);
  if (lnMatch) {
    ...
    response += 'I. DEFINITION\n';
    response += '   ln(x) = log_e(x)\n';
    response += '   e ≈ 2.71828\n\n';
    response += 'II. COMPUTATION\n';
    response += '   ln(' + value + ') = ' + result.toFixed(6) + '\n\n';
    response += 'III. FINAL ANSWER\n';
    response += '   ln(' + value + ') = ' + result.toFixed(4);
    return response;
  }
...
tryExponents(prompt) {
  ...
  if (exp < 0) {
    ...
    response += '   = ' + result.toFixed(6) + '\n\n';
    response += 'III. FINAL ANSWER\n';
    response += '   ' + base + '^' + exp + ' = ' + result.toFixed(6);
    return response;
  }
...
tryVector(prompt) {
  ...
  if (magMatch) {
    ...
    response += 'III. FINAL ANSWER\n';
    response += '   Magnitude = ' + mag.toFixed(4);
    return response;
  }
...
tryComplexNumber(prompt) {
  ...
  if (addMatch) {
    ...
    response += '   ' + a + ' ' + op + ' ' + c + ' = ' + real + '\n\n';
    response += 'II. COMBINE IMAGINARY PARTS\n';
    response += '   ' + b + 'i ' + op + ' ' + d + 'i = ' + imag + 'i\n\n';
    response += 'III. FINAL ANSWER\n';
    response += '   ' + real + (imag >= 0 ? ' + ' : ' - ') + Math.abs(imag) + 'i';
    return response;
  }
...
tryProbability(prompt) {
  ...
  if (diceMatch) {
    ...
    response += '   {1, 2, 3, 4, 5, 6} → 6 outcomes\n\n';
    response += 'II. FAVORABLE OUTCOMES\n';
    response += '   {(' + target + ')} → 1 outcome\n\n';
    response += 'III. FORMULA\n';
    response += '   P = favorable / total\n\n';
    response += 'IV. COMPUTATION\n';
    response += '   P = 1 / 6 = ' + prob.toFixed(6) + '\n\n';
    response += 'V. FINAL ANSWER\n';
    response += '   P = ' + prob.toFixed(6) + ' (' + (prob * 100).toFixed(2) + '%)';
    return response;
  }
...
tryChemistry(prompt) {
  ...
  if (!/(balance|molar mass|mole|ph|density|atomic)/i.test(lower)) return null;
  ...
  try {
    ...
    if (balanceMatch) {
      ...
      if (balanced) {
        ...
        response += '   ' + balanced + '\n\n';
        response += 'III. VERIFICATION\n';
        response += '   Count atoms on both sides - should be equal.';
        return response;
      }
    }
    ...
    if (moleMatch) {
      ...
      if (molarMass) {
        ...
        response += '   Molar Mass = ' + molarMass.total.toFixed(3) + ' g/mol\n\n';
        response += 'II. FORMULA\n';
        response += '   moles = mass / molar mass\n\n';
        response += 'III. COMPUTATION\n';
        response += '   moles = ' + mass + ' / ' + molarMass.total.toFixed(3) + '\n';
        response += '   moles = ' + moles.toFixed(4) + ' mol\n\n';
        response += 'IV. FINAL ANSWER\n';
        response += '   ' + moles.toFixed(4) + ' mol';
        return response;
      }
    }

    const phMatch = prompt.match(/ph\s+(?:of\s+)?(?:\[H\+\]\s*=\s*)?([\d\.]+)/i);
    if (phMatch) {
...
tryFinance(prompt) {
  ...
  try {
    ...
    if (siMatch) {
      ...
      response += '   Rate (r) = ' + (r * 100).toFixed(2) + '% = ' + r + '\n';
    ...
    if (ciMatch) {
      ...
      response += '   Rate (r) = ' + (r * 100).toFixed(2) + '% = ' + r + '\n';
      response += '   Time (t) = ' + t + ' years\n';
      response += '   Compounding (n) = ' + n + ' times per year\n\n';
      response += 'II. FORMULA\n';
      response += '   A = P(1 + r/n)^(nt)\n';
      response += '   I = A - P\n\n';
      response += 'III. COMPUTATION\n';
      response += '   A = ' + P + '(1 + ' + r + '/' + n + ')^(' + n + '×' + t + ')\n';
      response += '   A = ' + P + '(1 + ' + (r / n).toFixed(4) + ')^' + (n * t) + '\n';
      response += '   A = ' + P + ' × ' + Math.pow(1 + r / n, n * t).toFixed(4) + '\n';
      response += '   A = ' + A.toFixed(2) + '\n';
      response += '   I = ' + A.toFixed(2) + ' - ' + P + ' = ' + I.toFixed(2) + '\n\n';
      response += 'IV. FINAL ANSWER\n';
    ...
    if (roiMatch) {
      ...
      response += 'I. GIVEN\n';
      response += '   Initial Investment = ' + initial + '\n';
      response += '   Final Value = ' + finalVal + '\n\n';
      response += 'II. FORMULA\n';
      response += '   Profit = Final - Initial\n';
      response += '   ROI = (Profit / Initial) × 100%\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Profit = ' + finalVal + ' - ' + initial + ' = ' + profit + '\n';
      response += '   ROI = (' + profit + ' / ' + initial + ') × 100%\n';
      response += '   ROI = ' + roi.toFixed(2) + '%\n\n';
      response += 'IV. FINAL ANSWER\n';
      response += '   ROI = ' + roi.toFixed(2) + '%';
      return response;
    }
    ...
    if (profitMatch) {
      ...
      let response = 'FINANCE SOLUTION — PROFIT / MARKUP\n\n';
      response += 'I. GIVEN\n';
      response += '   Cost Price (CP) = ' + cp + '\n';
      response += '   Selling Price (SP) = ' + sp + '\n\n';
      response += 'II. FORMULAS\n';
      response += '   Profit = SP - CP\n';
      response += '   Markup = (Profit / CP) × 100%\n';
      response += '   Margin = (Profit / SP) × 100%\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Profit = ' + sp + ' - ' + cp + ' = ' + profit + '\n';
      response += '   Markup = (' + profit + ' / ' + cp + ') × 100% = ' + markup.toFixed(2) + '%\n';
      response += '   Margin = (' + profit + ' / ' + sp + ') × 100% = ' + margin.toFixed(2) + '%\n\n';
      response += 'IV. FINAL ANSWER\n';
    ...
    if (discMatch) {
      ...
      response += 'I. GIVEN\n';
      response += '   Original Price = ' + op + '\n';
      response += '   Discount Rate = ' + (disc * 100).toFixed(2) + '%\n\n';
      response += 'II. FORMULAS\n';
      response += '   Discount = OP × Rate\n';
      response += '   Final Price = OP - Discount\n\n';
      response += 'III. COMPUTATION\n';
...
tryAccounting(prompt) {
  ...
  if (!/accounting|balance sheet|income statement|assets|liabilities|equity|debit|credit/i.test(lower)) return null;
  if (!/\d/.test(prompt)) return null;
  ...
  try {
    ...
    if (aeMatch) {
      ...
      response += 'I. GIVEN\n';
      response += '   Assets = ' + A + '\n';
      response += '   Liabilities = ' + L + '\n\n';
      response += 'II. FORMULA\n';
      response += '   Assets = Liabilities + Equity\n';
      response += '   Equity = Assets - Liabilities\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Equity = ' + A + ' - ' + L + '\n';
      response += '   Equity = ' + E.toFixed(2) + '\n\n';
      response += 'IV. FINAL ANSWER\n';
      response += '   Equity = ' + E.toFixed(2);
      return response;
    }
    ...
    if (niMatch) {
      ...
      response += 'I. GIVEN\n';
      response += '   Revenue = ' + rev + '\n';
      response += '   Expenses = ' + exp + '\n\n';
      response += 'II. FORMULA\n';
      response += '   Net Income = Revenue - Expenses\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Net Income = ' + rev + ' - ' + exp + '\n';
      response += '   Net Income = ' + ni.toFixed(2) + '\n\n';
      response += 'IV. FINAL ANSWER\n';
      response += '   Net Income = ' + ni.toFixed(2);
    ...
    if (gpMatch) {
      ...
      response += '   COGS = ' + cogs + '\n\n';
      response += 'II. FORMULA\n';
      response += '   Gross Profit = Revenue - COGS\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Gross Profit = ' + rev + ' - ' + cogs + '\n';
      response += '   Gross Profit = ' + gp.toFixed(2) + '\n\n';
      response += 'IV. FINAL ANSWER\n';
      response += '   Gross Profit = ' + gp.toFixed(2);
      return response;
    }
...
tryNumberTheory(prompt) {
  ...
  try {
    ...
    if (primeMatch) {
      ...
      response += 'Number: ' + n + '\n\n';
      response += 'I. METHOD\n';
      response += '   Divide by smallest prime repeatedly.\n\n';
      response += 'II. FACTORIZATION\n';
      response += '   ' + n + ' = ' + factors.join(' × ') + '\n\n';
      response += 'III. FINAL ANSWER\n';
    ...
    if (lcmMatch) {
      ...
      response += 'Numbers: ' + a + ', ' + b + '\n\n';
      response += 'I. FORMULA\n';
      response += '   LCM(a,b) = (a × b) / GCD(a,b)\n\n';
      response += 'II. COMPUTATION\n';
      response += '   GCD(' + a + ', ' + b + ') = ' + gcd + '\n';
      response += '   LCM = (' + a + ' × ' + b + ') / ' + gcd + '\n';
      response += '   LCM = ' + (a * b) + ' / ' + gcd + '\n';
      response += '   LCM = ' + lcm + '\n\n';
      response += 'III. FINAL ANSWER\n';
      response += '   LCM(' + a + ', ' + b + ') = ' + lcm;
      return response;
    }
    ...
    if (modMatch) {
      ...
      response += 'I. DEFINITION\n';
      response += '   a mod b = remainder when a is divided by b\n\n';
      response += 'II. COMPUTATION\n';
      response += '   ' + a + ' ÷ ' + b + ' = ' + Math.floor(a / b) + ' remainder ' + result + '\n';
      response += '   Check: ' + Math.floor(a / b) + ' × ' + b + ' + ' + result + ' = ' + (Math.floor(a / b) * b + result) + '\n\n';
      response += 'III. FINAL ANSWER\n';
      response += '   ' + a + ' mod ' + b + ' = ' + result;
      return response;
    }
    ...
    if (isPrimeMatch) {
      ...
      response += 'Number: ' + n + '\n\n';
      response += 'I. METHOD\n';
      response += '   Check divisibility from 2 to sqrt(' + n + ') ≈ ' + Math.floor(Math.sqrt(n)) + '\n\n';
      response += 'II. FINAL ANSWER\n';
      response += '   ' + n + ' is ' + (isPrime ? 'PRIME' : 'COMPOSITE');
      return response;
    }
...
trySetTheory(prompt) {
  ...
  if (!/(union|intersection|complement|subset|superset|difference)/i.test(lower)) return null;
  if (!/[{}]|\{/.test(prompt)) return null;
  ...
  try {
    ...
    else if (operation === 'INTERSECTION') response += '   A ∩ B = elements in both A and B\n';
    else if (operation === 'DIFFERENCE') response += '   A - B = elements in A but not in B\n';
    response += '\nIII. FINAL ANSWER\n';
...
tryLogicGates(prompt) {
  ...
  const match = prompt.match(/(and|or|not|nand|nor|xor|xnor)\s*\(\s*([01])\s*(?:,\s*([01]))?\s*\)/i);
  if (!match) return null;
...
tryBinaryHex(prompt) {
  ...
  try {
    ...
    if (binToDec) {
      ...
      response += 'Binary: ' + bin + '\n\n';
      response += 'I. METHOD\n';
      response += '   Multiply each digit by power of 2.\n\n';
      response += 'II. COMPUTATION\n';
      let sum = 0;
      for (let i = 0; i < bin.length; i++) {
        const power = bin.length - 1 - i;
        const val = parseInt(bin[i]) * Math.pow(2, power);
        if (parseInt(bin[i]) === 1) {
          response += '   ' + bin[i] + ' × 2^' + power + ' = ' + val + '\n';
          sum += val;
        }
      }
      response += '\nIII. FINAL ANSWER\n';
    ...
    if (decToBin) {
      ...
      response += 'Decimal: ' + dec + '\n\n';
      response += 'I. METHOD\n';
      response += '   Divide by 2 repeatedly, record remainders.\n\n';
      response += 'II. COMPUTATION\n';
      let temp = dec;
      const steps = [];
      while (temp > 0) {
    ...
    if (hexToDec) {
      ...
      response += 'Hex: ' + hex + '\n\n';
      response += 'I. METHOD\n';
      response += '   Multiply each digit by power of 16.\n\n';
      response += 'II. FINAL ANSWER\n';
      response += '   Decimal: ' + dec;
      return response;
    }
...
tryUnitPrefix(prompt) {
  ...
  if (match) {
    ...
    response += 'III. FINAL ANSWER\n';
    response += '   ' + result.toFixed(6) + ' ' + match[3];
    return response;
  }
...
// ============================================================
// HANDLER 31: TRIGONOMETRY ADVANCED
// ============================================================
tryTrigAdvanced(prompt) {
  ...
  try {
    ...
    if (losMatch) {
      ...
      let response = 'TRIGONOMETRY ADVANCED — LAW OF SINES\n\n';
      response += 'I. GIVEN\n';
      response += '   a = ' + a + '\n';
      response += '   A = ' + losMatch[2] + '°\n';
      response += '   B = ' + losMatch[3] + '°\n\n';
      response += 'II. FORMULA\n';
      response += '   a / sin(A) = b / sin(B)\n';
      response += '   b = a × sin(B) / sin(A)\n\n';
      response += 'III. COMPUTATION\n';
    ...
    if (locMatch) {
      ...
      let response = 'TRIGONOMETRY ADVANCED — LAW OF COSINES\n\n';
      response += 'I. GIVEN\n';
      response += '   a = ' + a + '\n';
      response += '   b = ' + b + '\n';
      response += '   C = ' + locMatch[3] + '°\n\n';
      response += 'II. FORMULA\n';
      response += '   c² = a² + b² - 2ab·cos(C)\n\n';
      response += 'III. COMPUTATION\n';
    ...
    const invMatch = prompt.match(/(arcsin|arccos|arctan|asin|acos|atan)\s*\(?\s*([\-\d\.]+)\s*\)?/i);
    if (invMatch) {
      ...
      let response = 'TRIGONOMETRY ADVANCED — INVERSE\n\n';
      response += 'I. INPUT\n';
      response += '   ' + func + '(' + val + ')\n\n';
      response += 'II. COMPUTATION\n';
...
// ============================================================
// HANDLER 33: ADVANCED STATISTICS
// ============================================================
tryStatsAdvanced(prompt) {
  if (!prompt) return null;
  const lower = prompt.toLowerCase();

  if (!/(variance|standard deviation|sd)/i.test(lower)) return null;
  ...
  try {
    ...
    response += '\nIII. VARIANCE\n';
    response += '   σ² = Σ(x - μ)² / n\n';
    response += '   σ² = ' + variance.toFixed(4) + '\n\n';
    response += 'IV. STANDARD DEVIATION\n';
    response += '   σ = √' + variance.toFixed(4) + '\n';
    response += '   σ = ' + sd.toFixed(4) + '\n\n';
    response += 'V. FINAL ANSWER\n';
...
tryEngineering(prompt) {
  ...
  if (!/(stress|strain|youngs modulus|torque|beam|deflection|moment|inertia)/i.test(lower)) return null;
  if (!/\d/.test(prompt)) return null;
...
tryElectrical(prompt) {
  ...
  if (!/(resistance|parallel|series|capacitor|inductor|impedance|electrical power)/i.test(lower)) return null;
  if (!/\d/.test(prompt)) return null;
  ...
  try {
    ...
    if (parallelMatch) {
      ...
      response += 'III. COMPUTATION\n   1/Rtotal = ';
      response += nums.map(r => '1/' + r).join(' + ');
      response += '\n   1/Rtotal = ' + inverseSum.toFixed(6) + '\n';
      response += '   Rtotal = ' + total.toFixed(4) + ' Ω\n\n';
      response += 'IV. FINAL ANSWER\n   Rtotal = ' + total.toFixed(4) + ' Ω';
      return response;
    }
...
tryOptics(prompt) {
  ...
  if (!/(snell|refraction|refractive|critical angle|mirror|lens)/i.test(lower)) return null;
  if (!/\d/.test(prompt)) return null;
...
// ============================================================
// HANDLER 40: GEOGRAPHY (Distance between Coordinates)
// ============================================================
tryGeography(prompt) {
  if (!prompt) return null;
  const lower = prompt.toLowerCase();
  ...
  try {
    ...
    if (coordMatch) {
      ...
      response += '   R = 6371 km\n\n';
      response += 'III. COMPUTATION\n';
      response += '   Distance = ' + d.toFixed(2) + ' km\n\n';
      response += 'IV. FINAL ANSWER\n';
      response += '   Distance = ' + d.toFixed(2) + ' km';
      return response;
    }
  ...
  tryMeasurement(prompt) {
    ...
    for (const [key, factor] of Object.entries(conversions)) {
      ...
      if (match) {
        ...
        response += 'Problem: Convert ' + inputVal + ' ' + from + ' to ' + to + '\n\n';
        response += 'I. CONVERSION FACTOR\n';
        response += '   1 ' + from + ' = ' + factor.toFixed(6) + ' ' + to + '\n\n';
        response += 'II. COMPUTATION\n';
        response += '   ' + inputVal + ' × ' + factor.toFixed(6) + ' = ' + result.toFixed(4) + '\n\n';
        response += 'III. FINAL ANSWER\n';
        response += '   ' + inputVal + ' ' + from + ' = ' + result.toFixed(4) + ' ' + to;
        return response;
      }
  ...
  tryRoman(prompt) {
    ...
    if (romanMatch && /(roman|to decimal|convert|decimal)/i.test(lower)) {
      ...
      response += 'I. METHOD\n';
      response += '   If smaller value precedes larger, subtract.\n';
      response += '   Otherwise, add.\n\n';
      response += 'II. FINAL ANSWER\n';
    ...
    if (decMatch) {
      ...
      response += 'I. METHOD\n';
      response += '   Convert using largest Roman values first.\n\n';
      response += 'II. FINAL ANSWER\n';
      response += '   Roman: ' + result;
      return response;
    }
  ...
  tryBaseConversion(prompt) {
    ...
    try {
      ...
      response += 'From Base: ' + fromBase + '\n';
      response += 'To Base: ' + toBase + '\n\n';
      response += 'I. STEP 1: Convert to Decimal\n';
      response += '   ' + numStr + ' (base ' + fromBase + ') = ' + dec + ' (decimal)\n\n';
      response += 'II. STEP 2: Convert Decimal to Base ' + toBase + '\n';
      response += '   ' + dec + ' (decimal) = ' + result + ' (base ' + toBase + ')\n\n';
      response += 'III. FINAL ANSWER\n';
      response += '   ' + result;
      return response;
    } catch (e) {
    ...
    try {
      ...
      if (match) {
        ...
        if (new RegExp(var1 + '\\s*(?:OR|\\+)', 'i').test(inside) && new RegExp('(?:AND|\\*)', 'i').test(prompt)) {
          ...
          response += 'Expression: ' + prompt + '\n\n';
          response += 'I. LAW USED\n';
          response += '   Absorption Law: A + (A · B) = A\n';
          response += '   Or: A · (A + B) = A\n\n';
          response += 'II. FINAL ANSWER\n';
          response += '   ' + var1;
          return response;
  ...
  tryDistribution(prompt) {
    ...
    try {
      ...
      if (binomMatch) {
        ...
        response += '   k = ' + k + '\n\n';
        response += 'II. FORMULA\n';
        response += '   P(X=k) = C(n,k) × p^k × (1-p)^(n-k)\n\n';
        response += 'III. COMPUTATION\n';
        response += '   C(' + n + ',' + k + ') = ' + comb + '\n';
  ...
  tryDiffEq(prompt) {
    ...
    try {
      ...
      if (powerMatch) {
        ...
        response += '   y = ' + newCoeff.toFixed(4) + 'x^' + (n + 1) + ' + C\n\n';
        response += 'III. FINAL ANSWER\n';
        response += '   y = ' + newCoeff.toFixed(4) + 'x^' + (n + 1) + ' + C';
        return response;
      }
  ...
  tryGraphTheory(prompt) {
    ...
    if (degreeMatch) {
      ...
      response += 'Vertex: ' + v + '\n\n';
      response += 'I. DEFINITION\n';
      response += '   Degree of a vertex = number of edges connected to it.\n\n';
      response += 'II. NOTE\n';
      response += '   Please provide the edge list to count exactly.\n\n';
      response += 'III. FINAL ANSWER\n';
      response += '   Cannot compute without edge list.';
      return response;
    }
  ...
  // ============================================================
  // HANDLER 58: CRYPTOGRAPHY (Caesar Cipher, ROT13)
  // ============================================================
  ...
  tryDataStructures(prompt) {
    ...
    if (/stack/i.test(lower)) {
      response += 'I. STACK\n   LIFO: Last In, First Out\n\n';
      response += 'II. OPERATIONS\n   push(x) — add to top\n   pop() — remove from top\n   peek() — view top\n';
      return response;
    }

    if (/queue/i.test(lower)) {
  ...
  // ============================================================
  // API CALLS — 4 APIs (no Pollinations)
  // ============================================================
  ...
  applyUniversalFormat(response, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    const briefIntents = ['definition', 'general', 'explanation', 'process', 'list', 'comparison', 'howto', 'proscons', 'importance', 'effects', 'biography', 'wordlist', 'followup', 'elaborate', 'paraphrase', 'simplify', 'expand', 'correct', 'formalize', 'construct'];
  ...
  detectSubject(prompt) {
    ...
    if (/sin|cos|tan/i.test(lower)) return 'trigonometry';
