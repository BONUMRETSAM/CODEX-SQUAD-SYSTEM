// ========== ai.js - PRECISE AND CLEAR RESPONSES ==========
...
module.exports = {
  ...
  description: 'Complete AI assistant with precise and clear responses',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== CHECK IF PURE MATH EXPRESSION FIRST (STRICT) =====
  ...
  // ============================================================
  // SEND CONTINUOUS - Split but complete
  // ============================================================
  async sendContinuous(senderId, text, token) {
    ...
    console.log('[Continuous] Total chunks:', chunks.length);
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        console.error('[Continuous] Send error:', error.message);
  ...
  detectRequestType(prompt) {
    ...
    if (/summarize|summary|buod|i-summarize|i-buod|shorten|paikliin|make it short|brief|concise/i.test(lower)) {
      return 'summarize';
    }
    if (/short|concise|brief|maikli|iklian|mubo/i.test(lower)) {
      return 'short';
    }
    if (/translate|isalin|salin|ipasalin|translation/i.test(lower)) {
      return 'translate';
    }
    if (/^[\d\s\+\-\*\/\(\)\.\,×÷]+$/.test(lower.replace(/\s/g, ''))) {
      return 'math';
    }
    if (/explain|paliwanag|ipaliwanag|detail|elaborate|define|describe|process|steps|how does|how do|full|complete|ilarawan|buong|kompleto/i.test(lower)) {
      return 'detailed';
    }
    return 'general';
  },
  ...
  formatPreciseResponse(response, requestType) {
    ...
    if (requestType === 'summarize') {
      formatted = formatted
        ...
        .replace(/^Summary:.*?\n/i, '')
        .trim();
      if (!formatted.match(/^(Summary|Buod):/i)) {
        formatted = 'Summary: ' + formatted;
      }
    } else if (requestType === 'short') {
      formatted = formatted
        .replace(/^Here is a short.*?\n/i, '')
      ...
      formatted = formatted
        ...
        .replace(/^Translation:.*?\n/i, '')
        .trim();
      if (!formatted.match(/^(Translation|Salin):/i)) {
        formatted = 'Translation: ' + formatted;
      }
    } else if (requestType === 'detailed') {
      // ===== ENSURE PRECISE FORMAT =====
      // Ensure starts with "Answer:"
      if (!formatted.match(/^(Answer|Sagot):/i)) {
        formatted = 'Answer:\n' + formatted;
      }
      // Ensure has "Final Answer:" at the end for detailed responses
  ...
  tryMathCompute(prompt) {
    ...
    const nonMathKeywords = [
      'summarize', 'summary', 'buod', 'i-summarize', 'i-buod',
      'shorten', 'paikliin', 'make it short', 'brief', 'concise',
      'short', 'maikli', 'iklian', 'mubo',
      'translate', 'isalin', 'salin', 'ipasalin', 'translation',
      'explain', 'paliwanag', 'ipaliwanag', 'detail', 'elaborate',
      'what', 'ano', 'why', 'bakit', 'how', 'paano',
      'who', 'sino', 'when', 'kailan', 'where', 'saan',
      'which', 'alin', 'describe', 'ilarawan',
      'give', 'bigay', 'list', 'enumerate', 'isahin',
      'tell', 'sabi', 'write', 'isulat', 'make', 'gawa',
      'help', 'tulong', 'about', 'tungkol', 'regarding', 'ukol',
      'define', 'process', 'steps', 'full', 'complete'
    ];
  ...
  buildPrecisePrompt(prompt, previousResponse, previousPrompt, requestType) {
    ...
    finalPrompt += `You are a precise AI assistant. Follow these rules EXACTLY:\n`;
    finalPrompt += `1. Answer directly and accurately.\n`;
    finalPrompt += `2. NO introduction, NO conclusion.\n`;
    finalPrompt += `3. NO emojis, NO markdown, NO special symbols.\n`;
    finalPrompt += `4. Use plain text only.\n`;
    finalPrompt += `5. Be CLEAR and PRECISE.\n`;
    finalPrompt += `6. Use proper structure with numbered steps if needed.\n`;

    if (requestType === 'summarize') {
      finalPrompt += `\nThe user wants a SUMMARY.\n`;
      finalPrompt += `- Give 3-5 sentences summary.\n`;
      finalPrompt += `- Focus on KEY POINTS only.\n`;
      finalPrompt += `- Start with "Summary:".\n`;
    } else if (requestType === 'short') {
      finalPrompt += `\nThe user wants a SHORT answer.\n`;
      finalPrompt += `- Give 1-3 sentences only.\n`;
      finalPrompt += `- Be CONCISE.\n`;
      finalPrompt += `- Start with "Answer:".\n`;
    } else if (requestType === 'translate') {
      finalPrompt += `\nThe user wants a TRANSLATION.\n`;
      finalPrompt += `- Translate accurately.\n`;
      finalPrompt += `- Start with "Translation:".\n`;
    } else if (requestType === 'detailed') {
      finalPrompt += `\nCRITICAL: The user wants a FULL and DETAILED answer.\n`;
      finalPrompt += `- Start with "Answer:" then a clear definition.\n`;
      finalPrompt += `- Provide the FULL PROCESS step by step.\n`;
      finalPrompt += `- Use numbered steps (1, 2, 3) for each stage.\n`;
      finalPrompt += `- Use CAPITAL LETTERS for stage names.\n`;
      finalPrompt += `- Include chemical equations if applicable.\n`;
      finalPrompt += `- Include the overall equation if applicable.\n`;
      finalPrompt += `- End with "Final Answer:" for the summary.\n`;
      finalPrompt += `- Be COMPREHENSIVE, CLEAR, and PRECISE.\n`;
      finalPrompt += `- NO introduction, NO conclusion.\n`;
    } else {
      finalPrompt += `\nGive a CLEAR, PRECISE, and ACCURATE answer.\n`;
      finalPrompt += `- Use proper structure.\n`;
      finalPrompt += `- Be complete but not verbose.\n`;
    }
    ...
    finalPrompt += `Answer now. Be precise and clear.`;
