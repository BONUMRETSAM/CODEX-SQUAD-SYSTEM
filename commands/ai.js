...
module.exports = {
  ...
  description: 'Complete AI assistant with accurate detailed responses',
  ...
  // ============================================================
  // BUILD PROMPT - Simple and accurate
  // ============================================================
  buildPrompt(prompt, previousResponse, previousPrompt, requestType) {
    let finalPrompt = '';

    if (requestType === 'summarize') {
      finalPrompt += `Give a brief summary (3-5 sentences) of the following.\n`;
      finalPrompt += `Focus on the key points. Be clear and accurate.\n\n`;
    } else if (requestType === 'short') {
      finalPrompt += `Give a short, direct answer (1-3 sentences).\n`;
      finalPrompt += `Be accurate and clear.\n\n`;
    } else if (requestType === 'translate') {
      finalPrompt += `Translate the following accurately.\n\n`;
    } else if (requestType === 'detailed') {
      finalPrompt += `Give a COMPLETE and DETAILED answer.\n\n`;
      finalPrompt += `INCLUDE:\n`;
      finalPrompt += `1. Clear definition or introduction\n`;
      finalPrompt += `2. Full explanation with steps or key points\n`;
      finalPrompt += `3. Specific examples for each point\n`;
      finalPrompt += `4. Equations or formulas if applicable\n`;
      finalPrompt += `5. Final summary\n\n`;
      finalPrompt += `IMPORTANT:\n`;
      finalPrompt += `- Be ACCURATE and THOROUGH\n`;
      finalPrompt += `- Include ALL important details\n`;
      finalPrompt += `- Use plain text for equations\n`;
      finalPrompt += `- NO LaTeX symbols\n`;
      finalPrompt += `- NO markdown\n`;
      finalPrompt += `- NO emojis\n\n`;
    } else {
      finalPrompt += `Answer the following question accurately and clearly.\n`;
      finalPrompt += `Be complete but concise.\n\n`;
    }
  ...
  detectRequestType(prompt) {
    ...
    if (/define|explain|describe|process|steps|how does|how do|full|complete|detailed|elaborate|paliwanag|ipaliwanag|ilarawan|buong|kompleto|examples/i.test(lower)) return 'detailed';
    return 'general';
  },
