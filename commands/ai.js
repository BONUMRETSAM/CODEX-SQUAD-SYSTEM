// ========== ai.js - COMPLETE AI ASSISTANT v37.0.0 ==========
// Full Fix: Walang Pollinations, walang credit error, kumpleto lahat ng functions
...
module.exports = {
  ...
  description: 'Complete AI assistant with 4 working APIs',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (isFollowUpCommand) {
        ...
        const response = await this.callAPIWithFallback(followUpPrompt, prompt, 'english', action, action, 'followup', 'detailed');
      ...
      if (this.isTranslationQuestion(prompt)) {
        ...
        if (translatedAnswer) {
          ...
          conversationHistory[senderId] = { lastPrompt: prompt, lastResponse: translatedAnswer, timestamp: Date.now() };
          return;
        }
      }
      ...
      const response = await this.callAPIWithFallback(finalPrompt, prompt, language, intent, topic, subject, requestType);
      ...
      await sendMessage(senderId, {
        text: 'Sorry, hindi available ang AI service ngayon.\n\nSubukan muli mamaya.'
      }, token);
    }
  },

  // ============================================================
  // CALL API WITH FALLBACK (unified)
  // ============================================================
  async callAPIWithFallback(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    try {
      return await this.callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType);
    } catch (error) {
      console.log('[AI] Primary failed, using fallback...');
      return await this.callAPIFallback(prompt, originalPrompt, language, intent, topic, subject, requestType);
    }
  },

  // ============================================================
  // CALL API — 4 working APIs
  // ============================================================
  async callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    const errorKeywords = [
      ...
      'enter.pollinations', 'top-up?ref', 'quests?ref'
    ];
    ...
    for (const config of apiConfigs) {
      try {
        ...
        if (!extracted || typeof extracted !== 'string' || extracted.trim().length === 0) continue;
        ...
        if (quality.isGood && quality.score >= 65) return formatted;
  ...
  // ============================================================
  // CALL API FALLBACK — retry 4 APIs
  // ============================================================
  async callAPIFallback(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    const fallbackApis = [
      { name: 'DeepAI', url: 'https://ceddsrestapi.vercel.app/ai/deepai', param: 'message', responsePath: 'result' },
      { name: 'Overchat Qwen', url: 'https://ceddsrestapi.vercel.app/ai/overchat-qwen', param: 'message', responsePath: 'result', successField: 'operator', successValue: 'Ioarkdev' },
      { name: 'Cedds ChatPlus', url: 'https://ceddsrestapi.vercel.app/ai/chatplus', param: 'message', responsePath: 'result', successField: 'operator', successValue: 'ceddsdev' },
      { name: 'Opera AI', url: 'https://betadash-api-swordslush-production.up.railway.app/opera', param: 'ask', responsePath: 'message', successField: 'success' }
    ];

    const errorKeywords = [
      'not enough credits', 'low balance', 'top up', 'top-up', 'topup',
      'credits', 'balance', 'api key', 'quota', 'exceeded', 'pollinations',
      'enter.pollinations', 'quests', 'complete a quest'
    ];

    let bestResponse = null;
    let bestScore = -1;

    for (const config of fallbackApis) {
      try {
        const encodedPrompt = encodeURIComponent(prompt);
        const apiUrl = config.url + '?' + config.param + '=' + encodedPrompt;
        ...
        if (config.successField && data[config.successField] !== config.successValue) continue;

        const extracted = this.extractResponse(data, config);
        if (!extracted || extracted.trim().length === 0) continue;
        ...
        const formatted = this.applyUniversalFormat(extracted, originalPrompt, language, intent, topic, subject, requestType);
        const quality = this.validateResponseQuality(formatted, intent);
        console.log('[Fallback] ' + config.name + ' quality: ' + quality.score + '/100');
  ...
  // ============================================================
  // MATH — Proper MDAS with auto-final-answer
  // ============================================================
  tryMathCompute(prompt) {
    ...
    let normalized = prompt.replace(/÷/g, '/').replace(/×/g, '*').replace(/−/g, '-').replace(/,/g, '').trim();

    if (!/^[\d\s\+\-\*\/\(\)\.]+$/.test(normalized.replace(/\s/g, ''))) return null;

    const numbers = normalized.match(/\d+\.?\d*/g);
    if (!numbers || numbers.length < 2) return null;
    if (!/[\+\-\*\/]/.test(normalized)) return null;
    if (numbers.length === 1 && /^\d{4}$/.test(normalized)) return null;
    if (/^\d{1,2}\/\d{1,2}\/\d{2,4}$/.test(normalized)) return null;

    try {
      let expression = normalized.replace(/\s+/g, '');
      const result = Function('"use strict"; return (' + expression + ')')();
      if (typeof result !== 'number' || !isFinite(result)) return null;

      const steps = this.buildMDASSteps(normalized);

      let response = 'MATH SOLUTION\n\n';
      response += 'Expression: ' + normalized + '\n\n';
  ...
  buildMDASSteps(expression) {
    ...
    if (!tokens || tokens.length < 3) return ['Compute: ' + expression];
    ...
    while (i < tokensArr.length) {
      ...
      if (token === '*' || token === '/') {
        const left = parseFloat(tokensArr[i - 1]);
        const right = parseFloat(tokensArr[i + 1]);
        let result;
        ...
        tokensArr.splice(i - 1, 3, result.toString());
        steps.push('Step (M/D): ' + left + ' ' + (token === '*' ? 'x' : '÷') + ' ' + right + ' = ' + result);
        i = 0;
      } else { i++; }
    }
    ...
    while (i < tokensArr.length) {
      ...
      if (token === '+' || token === '-') {
        ...
        tokensArr.splice(i - 1, 3, result.toString());
        steps.push('Step (A/S): ' + left + ' ' + token + ' ' + right + ' = ' + result);
        i = 0;
      } else { i++; }
    }
  ...
  async handleTranslation(senderId, prompt, token) {
    try {
      ...
      const languageMap = {
        ...
        'urdu': 'Urdu', 'bengali': 'Bengali', 'swahili': 'Swahili', 'zulu': 'Zulu'
      };

      let targetLang = 'English';
      for (const [key, value] of Object.entries(languageMap)) {
        if (lower.includes(key)) { targetLang = value; break; }
      }
      ...
      for (const pattern of extractPatterns) {
        ...
        if (match) { phrase = (match[2] || match[1]).trim().replace(/[?.!]+$/, ''); break; }
      }
      ...
      finalPrompt += 'If the phrase is an IDIOM or EXPRESSION, translate the MEANING, not the words.\n\n';
      finalPrompt += 'CRITICAL RULES:\n';
      finalPrompt += '1. The phrase below is NOT the topic. It is the TEXT to translate.\n';
      finalPrompt += '2. Do NOT define the phrase. Do NOT explain the phrase.\n';
      finalPrompt += '3. Do NOT write TITLE, INTRODUCTION, DEFINITION, or any essay format.\n';
      finalPrompt += '4. Do NOT hallucinate. If unsure of a translation, do NOT include it.\n';
      finalPrompt += '5. Give ONLY translations that are ACTUALLY USED in the target language.\n';
      finalPrompt += '6. For idiomatic phrases, use IDIOMATIC equivalents, NOT literal translation.\n';
      finalPrompt += '7. Output ONLY the translations. No labels, no headers, no numbers.\n';
      finalPrompt += '8. Put each translation on its own line.\n';
      finalPrompt += '9. Give the PRIMARY (most common) translation on the FIRST LINE.\n';
      finalPrompt += '10. Then give ALL other valid translations. Give 15-25+ if available.\n';
      finalPrompt += '11. Do NOT stop at 5. If there are 20 valid translations, give all 20.\n';
      finalPrompt += '12. Include LITERAL, IDIOMATIC, FORMAL, CASUAL, SLANG, REGIONAL variants.\n';
      finalPrompt += '13. Be TUMPAK (accurate). Verify each translation is actually used.\n';
      finalPrompt += '14. NO emojis, NO markdown, NO LaTeX.\n\n';
      finalPrompt += 'PHRASE TO TRANSLATE: "' + phrase + '"\n\n';
      finalPrompt += 'Now translate "' + phrase + '" into ' + targetLang + '. Output ONLY the translations, one per line:';

      const response = await this.callAPIWithFallback(finalPrompt, prompt, 'english', 'translation', 'translation', 'translation', 'general');
      let cleaned = this.finalClean(response);
      ...
      cleaned = cleaned.replace(/^ALTERNATIVES?:\s*/gim, '');
      cleaned = cleaned.replace(/^OTHER\s+TRANSLATIONS?:\s*/gim, '');
      cleaned = cleaned.replace(/^PRIMARY\s+TRANSLATION:\s*/gim, '');
  ...
  async handleWordList(senderId, prompt, token) {
    try {
      ...
      else if (/all\s+(terms?|words?|names?)|list\s+of|enumerate/i.test(lower)) listType = 'list';
      ...
      let formatHint = '1. [synonym] - [brief meaning]';
      if (listType === 'antonym') { actionLabel = 'antonyms'; formatHint = '1. [antonym] - [brief meaning]'; }
      else if (listType === 'list') { actionLabel = 'terms'; formatHint = '1. [term] - [brief meaning]'; }
      ...
      finalPrompt += '7. Group by category if applicable (General, Related, Formal, Casual, Filipino, Bisaya).\n';
      finalPrompt += '8. Aim for 20-35 valid items if possible. Do NOT stop at 5.\n';
      finalPrompt += '9. Be TUMPAK (accurate). Only include words that are ACTUALLY valid.\n';
      finalPrompt += '10. NO emojis, NO markdown.\n\n';
      finalPrompt += 'WORD: ' + targetWord + '\n\n';
      finalPrompt += 'FORMAT:\n';
      finalPrompt += actionLabel.toUpperCase() + ' OF "' + targetWord.toUpperCase() + '"\n\n';
      finalPrompt += 'I. GENERAL ' + actionLabel.toUpperCase() + '\n';
      ...
      const response = await this.callAPIWithFallback(finalPrompt, prompt, 'english', 'wordlist', 'wordlist', 'wordlist', 'general');
      let cleaned = this.finalClean(response);
  ...
  isFollowUpCommand(cleanPrompt) {
    ...
    if (/^(elaborate|explain)\s+(and|at)\s+(give|show)\s+(full|complete|detailed)\s+(solution|answer|explanation)/i.test(p)) return true;
    if (/^(give|show)\s+(full|complete|detailed)\s+(solution|answer|explanation)/i.test(p)) return true;
    if (/^(full|complete|detailed)\s+(solution|answer|explanation)/i.test(p)) return true;
  ...
  getFollowUpAction(cleanPrompt) {
    ...
    if (/give.*full.*solution|full.*solution|complete.*solution|detailed.*solution/i.test(p)) return 'elaborate';
    if (/elaborate.*and.*give|elaborate.*solution/i.test(p)) return 'elaborate';
  ...
  buildFollowUpPrompt(action, previousResponse, userCommand) {
    const actionInstructions = {
      'elaborate': 'ELABORATE the following response. Add MORE DETAILS, examples, explanations, and deeper analysis. Keep the same structure but make it richer and more comprehensive. ALWAYS INCLUDE THE FINAL ANSWER at the end.',
      'summarize': 'SUMMARIZE the following response. Make it SHORT and CONCISE. Keep ONLY the key points. Use a MAXIMUM of 200 words. Remove all unnecessary details. KEEP the final answer.',
      ...
      'expand': 'EXPAND the following response. Add MORE INFORMATION, examples, and sub-points. Be COMPREHENSIVE — do NOT stop at 3 items.',
      'translate': 'TRANSLATE the following response into TAGALOG. If the phrase is idiomatic, use the IDIOMATIC equivalent. Output ONLY the translation, nothing else.',
    ...
    const isMathResponse = /step\s+\d|÷|×|\d\s*[\+\-\*\/]\s*\d/i.test(previousResponse);
    if (isMathResponse) {
      prompt += 'IMPORTANT FOR MATH:\n';
      prompt += '- The original response is a MATH SOLUTION.\n';
      prompt += '- Show ALL steps clearly.\n';
      prompt += '- Follow MDAS/PEMDAS strictly.\n';
      prompt += '- ALWAYS end with "Final Answer: [number]".\n';
      prompt += '- Do NOT change the final answer.\n\n';
    }
    ...
    prompt += '7. Be ACCURATE and COMPLETE. Do NOT cut corners.\n';
    prompt += '8. ALWAYS INCLUDE THE FINAL ANSWER.\n';
    prompt += '9. NO emojis, NO markdown, NO LaTeX.\n\n';

    if (action === 'summarize') {
      prompt += 'IMPORTANT FOR SUMMARIZE:\n- Output ONLY the summary.\n- Maximum 200 words.\n- KEEP the final answer.\n\n';
    }
    if (action === 'elaborate') {
      prompt += 'IMPORTANT FOR ELABORATE:\n- Add MORE STEPS if possible.\n- Explain each step in detail.\n- ALWAYS END WITH THE FINAL ANSWER.\n\n';
    }
  ...
  shouldTriggerWeight(lowerPrompt, originalPrompt) {
    ...
    const animalKeywords = ['pig', 'baboy', 'chicken', 'manok', 'cow', 'baka', 'kalabaw', 'carabao', 'goat', 'kambing', 'sheep', 'tupa', 'fish', 'isda', 'tilapia', 'bangus', 'duck', 'pato', 'itik', 'turkey', 'pabo', 'horse', 'kabayo', 'dog', 'aso', 'cat', 'pusa', 'wood', 'kahoy', 'tabla', 'rice', 'bigas', 'corn', 'mais', 'feeds', 'feed', 'hayop', 'alaga'];
  ...
  isLogicQuestion(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    const logicKeywords = ['riddle', 'bugtong', 'puzzle', 'logic', 'trick question', 'how many people', 'how many are', 'how many in the family', 'each son has', 'each daughter has', 'each brother has', 'each sister has', 'ilang tao', 'ilan ang tao', 'ilang miyembro', 'ilan ang pamilya'];
  ...
  applyUniversalFormat(response, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    const briefIntents = ['definition', 'general', 'explanation', 'process', 'list', 'comparison', 'howto', 'proscons', 'importance', 'effects', 'biography', 'wordlist', 'translation', 'followup', 'elaborate', 'summarize', 'paraphrase', 'simplify', 'expand', 'translate', 'correct', 'humanize', 'formalize', 'construct', 'definition_process'];
  ...
  validateResponseQuality(response, intent) {
    ...
    if (intent === 'definition_process' || intent === 'process') {
      const stages = response.match(/(Stage|Step|Phase)\s+\d+/gi);
      if (stages && stages.length >= 5) score += 20;
      else if (stages && stages.length >= 3) score += 10;
      else score -= 15;
    }
    if (intent === 'definition') {
      const bullets = response.match(/^[-*]\s+/gm);
      const numbers = response.match(/^\d+\.\s+/gm);
      const points = (bullets ? bullets.length : 0) + (numbers ? numbers.length : 0);
      if (points >= 3) score += 10;
      if (response.length >= 100 && response.length <= 800) score += 10;
      if (/is\s+a|is\s+the|refers\s+to|means/i.test(response)) score += 10;
    }
    if (intent === 'translation') {
      const lines = response.split('\n').filter(l => l.trim().length > 0);
      if (lines.length >= 15) score += 20;
      else if (lines.length >= 10) score += 15;
      else if (lines.length >= 5) score += 5;
      else score -= 20;
    }
    if (intent === 'wordlist') {
    ...
    if (intent === 'logic') {
      if (/REASONING/i.test(response)) score += 10;
      if (/ANSWER/i.test(response)) score += 10;
    }
    const briefIntents = ['definition', 'general', 'explanation', 'process', 'list', 'comparison', 'howto', 'proscons', 'importance', 'effects', 'biography', 'wordlist', 'translation', 'followup', 'elaborate', 'summarize', 'paraphrase', 'simplify', 'expand', 'translate', 'correct', 'humanize', 'formalize', 'construct', 'definition_process'];
    if (briefIntents.includes(intent) && response.length > 3000) score -= 30;
    return { isGood: score >= 60, isAcceptable: score >= 30, score: Math.max(0, score) };
  },
  ...
  detectSubject(prompt) {
    ...
    if (/(define|explain|describe|discuss|what is|what are|ano ang)\s+.+\s+(and|at)\s+(its|the|ang)\s+(process|processes|steps|stages|cycle|phases|procedure)/i.test(lower)) return 'definition_process';
    if (/\bcycle\s+(process|processes|stages|phases|steps)/i.test(lower)) return 'definition_process';
    if (/\b(process|processes|stages|phases|steps)\s+of\s+(the\s+)?cycle\b/i.test(lower)) return 'definition_process';
    if (/^what\s+(is|are)\s+the\s+(cycle|process|processes|stages|phases|steps)\s+(of|for)\s+/i.test(lower)) return 'definition_process';
    if (/^how\s+does\s+.+\s+(work|cycle|process|function)/i.test(lower)) return 'definition_process';
  ...
  detectIntent(prompt) {
    ...
    if (/\b(process|processes|stages|phases|steps)\s+of\s+(the\s+)?cycle\b/i.test(lower)) return 'definition_process';
    if (/^what\s+(is|are)\s+the\s+(cycle|process|processes|stages|phases|steps)\s+(of|for)\s+/i.test(lower)) return 'definition_process';
    if (/^what\s+(is|are)\s+the\s+(cycle|process|processes|stages|phases|steps)\s+(in|during)\s+/i.test(lower)) return 'definition_process';
    if (/^explain\s+the\s+(cycle|process|stages|phases|steps)\s+(of|for)\s+/i.test(lower)) return 'definition_process';
    if (/^describe\s+the\s+(cycle|process|stages|phases|steps)\s+(of|for)\s+/i.test(lower)) return 'definition_process';
    if (/^how\s+does\s+.+\s+(work|cycle|process|function)/i.test(lower)) return 'definition_process';
    if (/\b(stages|phases|steps)\s+of\s+\w+/i.test(lower) && /^(what|ano|explain|describe|discuss)/i.test(lower)) return 'definition_process';
    if (/^(define|explain|describe|discuss|what is|what are|ano ang)\s+.+\s+(and|at)\s+(its|the|ang)\s+(process|processes|steps|stages|cycle|phases|procedure)/i.test(lower)) return 'definition_process';
  ...
  detectRequestType(prompt) {
    ...
    if (/\bcycle\s+(process|processes|stages|phases|steps)/i.test(lower)) return 'detailed';
    if (/\b(process|processes|stages|phases|steps)\s+of\s+(the\s+)?cycle\b/i.test(lower)) return 'detailed';
    if (/^what\s+(is|are)\s+the\s+(cycle|process|processes|stages|phases|steps)\s+(of|for)\s+/i.test(lower)) return 'detailed';
    if (/^how\s+does\s+.+\s+(work|cycle|process|function)/i.test(lower)) return 'detailed';
    if (/(define|explain|describe|discuss|what is|what are|ano ang)\s+.+\s+(and|at)\s+(its|the|ang)\s+(process|processes|steps|stages|cycle|phases|procedure)/i.test(lower)) return 'detailed';
  ...
  extractTopic(prompt) {
    ...
    const actionPatterns = [
      ...
      /^what\s+(is|are)\s+the\s+(cycle|process|processes|stages|phases|steps)\s+(of|for|in|during)\s+/i,
      /^explain\s+the\s+(cycle|process|stages|phases|steps)\s+(of|for)\s+/i,
      /^describe\s+the\s+(cycle|process|stages|phases|steps)\s+(of|for)\s+/i,
  ...
  buildDetailedPrompt(prompt, previousResponse, previousPrompt, language, intent, topic, subject, requestType) {
    ...
    if (intent === 'definition_process') return this.buildDefinitionProcessPrompt(prompt, langName);
    return this.buildBriefPrompt(prompt, langName, intent);
  },

  buildDefinitionProcessPrompt(prompt, langName) {
    const lower = prompt.toLowerCase();
    const isCycle = /cycle|cycles|cyclic|stage|stages|phase|phases/i.test(lower);
    let finalPrompt = '';
    finalPrompt += 'You are a helpful AI assistant. Give a COMPLETE, ACCURATE, and COMPREHENSIVE answer.\n\n';
    finalPrompt += 'CRITICAL RULES:\n';
    finalPrompt += '1. Do NOT write an essay, thesis, or research paper.\n';
    finalPrompt += '2. Do NOT use these section headers: TITLE, INTRODUCTION, BACKGROUND OF THE STUDY, STATEMENT OF THE PROBLEM, OBJECTIVES, SIGNIFICANCE, SCOPE AND DELIMITATION, REVIEW OF RELATED LITERATURE, THEORETICAL FRAMEWORK, CONCEPTUAL FRAMEWORK, METHODOLOGY, RESEARCH DESIGN, RESPONDENTS, PARTICIPANTS, INSTRUMENT, DATA GATHERING, DATA COLLECTION, DATA ANALYSIS, STATISTICAL TREATMENT, RESULTS AND DISCUSSION, RECOMMENDATIONS, REFERENCES.\n';
    finalPrompt += '3. Do NOT use TITLE: at the start.\n';
    finalPrompt += '4. Do NOT repeat the question.\n';
    finalPrompt += '5. Be ACCURATE and PRECISE. Verify each fact. Do NOT hallucinate.\n';
    finalPrompt += '6. Be COMPREHENSIVE. Cover ALL stages/phases/steps. Do NOT stop at 3 items.\n';
    finalPrompt += '7. Be TUMPAK (accurate). If a stage/step is unclear, be specific.\n';
    finalPrompt += '8. Answer ONLY what is asked.\n';
    finalPrompt += '9. Do NOT pad with generic filler.\n';
    finalPrompt += '10. Respond in ' + langName.toUpperCase() + ' language.\n';
    finalPrompt += '11. NO emojis.\n\n';
    if (isCycle) {
      finalPrompt += 'FORMAT (CYCLE / STAGES):\nI. DEFINITION\n[2-4 sentences definition]\n\nII. KEY CHARACTERISTICS\n1. [characteristic]\n2. [characteristic]\n3. [characteristic]\n4. [characteristic]\n\nIII. CYCLE STAGES / PROCESSES\nStage 1: [name]\n[1-3 sentences description]\nStage 2: [name]\n[1-3 sentences description]\nStage 3: [name]\n[1-3 sentences description]\nStage 4: [name]\n[1-3 sentences description]\nStage 5: [name] (if applicable)\n[1-3 sentences description]\nStage 6: [name] (if applicable)\n[1-3 sentences description]\n\nIV. IMPORTANCE\n1. [importance]\n2. [importance]\n3. [importance]\n4. [importance]\n\nV. SUMMARY\n[2-3 sentences summary]\n\n';
    } else {
      finalPrompt += 'FORMAT (PROCESS / STEPS):\nI. DEFINITION\n[2-4 sentences definition]\n\nII. KEY CHARACTERISTICS\n1. [characteristic]\n2. [characteristic]\n3. [characteristic]\n4. [characteristic]\n\nIII. PROCESSES / STEPS\n1. [step]\n[1-2 sentences description]\n2. [step]\n[1-2 sentences description]\n3. [step]\n[1-2 sentences description]\n4. [step]\n[1-2 sentences description]\n5. [step]\n[1-2 sentences description]\n\nIV. EXAMPLES\n1. [example]\n2. [example]\n3. [example]\n\nV. IMPORTANCE\n1. [importance]\n2. [importance]\n3. [importance]\n4. [importance]\n\nVI. SUMMARY\n[2-3 sentences summary]\n\n';
    }
  ...
  buildBriefPrompt(prompt, langName, intent) {
    let finalPrompt = '';
    finalPrompt += 'You are a helpful AI assistant. Answer DIRECTLY, CONCISELY, ACCURATELY, and COMPLETELY.\n\n';
    finalPrompt += 'CRITICAL RULES:\n';
    finalPrompt += '1. Answer ONLY what is asked.\n';
    finalPrompt += '2. Do NOT write an essay, thesis, or research paper.\n';
    finalPrompt += '3. Do NOT use TITLE, INTRODUCTION, BACKGROUND OF THE STUDY, STATEMENT OF THE PROBLEM, OBJECTIVES, SIGNIFICANCE, SCOPE AND DELIMITATION, REVIEW OF RELATED LITERATURE, THEORETICAL FRAMEWORK, CONCEPTUAL FRAMEWORK, METHODOLOGY, RESEARCH DESIGN, RESPONDENTS, PARTICIPANTS, INSTRUMENT, DATA GATHERING, DATA COLLECTION, DATA ANALYSIS, STATISTICAL TREATMENT, RESULTS AND DISCUSSION, RECOMMENDATIONS, REFERENCES.\n';
    finalPrompt += '4. Do NOT use TITLE: at the start.\n';
    finalPrompt += '5. Do NOT repeat the question.\n';
    finalPrompt += '6. Do NOT hallucinate. Be ACCURATE and PRECISE. Verify each fact.\n';
    finalPrompt += '7. Be COMPLETE. Cover ALL important points. Do NOT stop at 2-3 items if more exist.\n';
    finalPrompt += '8. Do NOT pad with generic filler.\n';
    finalPrompt += '9. ALWAYS END WITH A COMPLETE ANSWER.\n';
    finalPrompt += '10. NO emojis, NO markdown, NO LaTeX.\n';
    finalPrompt += '11. Respond in ' + langName.toUpperCase() + ' language.\n\n';
  ...
  buildLogicPrompt(prompt, langName) {
    ...
    finalPrompt += '3. Do NOT hallucinate.\n';
    finalPrompt += '4. If it is a TRICK question, explain the trick clearly.\n';
    finalPrompt += '5. Do NOT repeat the question.\n';
    finalPrompt += '6. Give the FINAL ANSWER clearly at the end.\n';
    finalPrompt += '7. Verify your reasoning before giving the answer.\n';
    finalPrompt += '8. Be TUMPAK (accurate).\n';
    finalPrompt += '9. Respond in ' + langName.toUpperCase() + ' language.\n';
    finalPrompt += '10. Keep it CONCISE.\n';
    finalPrompt += '11. NO emojis, NO markdown, NO LaTeX.\n\n';
    finalPrompt += 'FORMAT:\nLogic Answer\n\nI. REASONING\n[Step-by-step reasoning]\n\nII. ANSWER\n[Final answer]\n\n';
    finalPrompt += 'QUESTION: ' + prompt + '\n\nWrite your ACCURATE response now:';
    return finalPrompt;
  },

  buildWordListPrompt(prompt, langName) {
    return 'You are a dictionary expert. Give ALL common synonyms/terms for the word in the question.\n\nCRITICAL RULES:\n1. Output ONLY a NUMBERED LIST.\n2. Do NOT write INTRODUCTION, DEFINITION, MAIN POINTS, EXAMPLES, IMPORTANCE, or CONCLUSION.\n3. Do NOT write any essay or paragraph.\n4. Just list the terms directly.\n5. Do NOT hallucinate. Only include valid words.\n6. Be COMPLETE. Aim for 20-35 items if possible.\n7. Respond in ' + langName.toUpperCase() + ' language.\n8. NO emojis, NO markdown.\n\nQUESTION: ' + prompt + '\n\nGive the list directly:';
  },

  buildTranslationPrompt(prompt, langName) {
    return 'You are a professional TRANSLATOR fluent in ALL languages.\n\nTASK: Translate the phrase below into ' + langName.toUpperCase() + '.\nIf the phrase is an IDIOM or EXPRESSION, translate the MEANING, not the words.\n\nCRITICAL RULES:\n1. The phrase below is NOT the topic. It is the TEXT to translate.\n2. Do NOT define the phrase.\n3. Do NOT write any essay format.\n4. Do NOT add labels like "TRANSLATION IN..." or "ALTERNATIVE TRANSLATIONS:".\n5. Output ONLY the translation.\n6. Give the PRIMARY translation first.\n7. Then give ALL POSSIBLE alternative translations. Do NOT limit to 3-5.\n8. Give 15-25+ translations if available.\n9. Put each translation on its own line.\n10. Do NOT hallucinate. Only include valid translations.\n11. NO emojis, NO markdown, NO LaTeX.\n\nQUESTION: ' + prompt + '\n\nGive the translation directly:';
  },

  // ============================================================
  // SEND COMPLETE — with part indicators
  // ============================================================
  async sendComplete(senderId, text, token) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (total > 1) {
        chunk = 'Part ' + (i + 1) + '/' + total + '\n\n' + chunk;
      }
      
      if (i < chunks.length - 1) {
        chunk = chunk.trim() + '\n\n[Continue sa Part ' + (i + 2) + '...]';
      }
      
      if (i === chunks.length - 1 && total > 1) {
        chunk = chunk.trim() + '\n\n[End of response]';
      }
  ...
  // ============================================================
  // SPLIT COMPLETE — section/sentence boundary
  // ============================================================
  splitComplete(text) {
    if (!text) return [];
  ...
  // ============================================================
  // ENSURE COMPLETE — auto-add final answer
  // ============================================================
  ensureComplete(text) {
    if (!text) return text;
    ...
    const isMath = /step\s+\d|÷|×|\d\s*[\+\-\*\/]\s*\d|final answer/i.test(text);
    
    if (isMath && !/final answer|answer:|answer is/i.test(text)) {
      const mathExpr = text.match(/(\d[\d\s\+\-\*\/÷×\(\)\.]+)/);
      if (mathExpr) {
        try {
          const expr = mathExpr[1].replace(/÷/g, '/').replace(/×/g, '*');
  ...
  isFollowUpRequest(prompt) {
    const keywords = ['elaborate', 'explain more', 'paki elaborate', 'paliwanag', 'ipaliwanag', 'detail', 'further', 'more details', 'summarize', 'summary', 'brief', 'short', 'concise', 'simplify', 'simple', 'example', 'sample', 'halimbawa', 'correct', 'fix', 'tama', 'add', 'dagdagan', 'more', 'humanize', 'translate', 'isalin', 'ulit', 'repeat'];
    return keywords.some(keyword => prompt.includes(keyword));
  },

  isContextualQuestion(prompt, previousPrompt) {
    if (!previousPrompt) return false;
    const patterns = ['so yan', 'so ito', 'tama ba', 'so tungkol', 'so ibig sabihin', 'paano naman', 'what about', 'bakit', 'why', 'paano', 'how', 'sino', 'who', 'alin', 'which', 'ano', 'what', 'gets', 'ok', 'sige', 'talaga', 'sure'];
  ...
  isNewTopic(prompt, previousPrompt) {
    ...
    const indicators = ['hello', 'hi', 'hey', 'kamusta', 'musta', 'ask', 'tanong', 'question', 'new topic', 'bagong topic', 'change topic', 'ibang topic', 'what is', 'ano ang', 'tell me about', 'explain', 'define', 'describe'];
  ...
  // ============================================================
  // EXTRACT RESPONSE — reject credit error
  // ============================================================
  extractResponse(data, config) {
    ...
    const dataStr = JSON.stringify(data).toLowerCase();
    if (errorKeywords.some(kw => dataStr.includes(kw))) return null;
    ...
    if (config.responsePath) {
      ...
      if (value && typeof value === 'string' && value.trim()) {
        ...
        if (errorKeywords.some(kw => valLower.includes(kw))) return null;
        return value;
      }
    }
    ...
    if (typeof data === 'string' && data.trim()) {
      ...
      if (errorKeywords.some(kw => dataLower.includes(kw))) return null;
      return data;
    }
    return null;
  },
  ...
  // ============================================================
  // CLEAN RESPONSE — remove credit error
  // ============================================================
  cleanResponse(text) {
    ...
    cleaned = cleaned.replace(/^OTHER\s+TRANSLATIONS?:\s*/gim, '');
    cleaned = cleaned.replace(/^PRIMARY\s+TRANSLATION:\s*/gim, '');
    cleaned = cleaned.replace(/The account behind this API key[\s\S]*?using\./gi, '');
    cleaned = cleaned.replace(/Please \[top up\][\s\S]*?try again\./gi, '');
    cleaned = cleaned.replace(/pollinations\.ai\/top-up/gi, '');
    cleaned = cleaned.replace(/pollinations\.ai\/quests/gi, '');
    cleaned = cleaned.replace(/enter\.pollinations\.ai/gi, '');
    cleaned = cleaned.replace(/complete a quest/gi, '');
    cleaned = cleaned.replace(/If this isn.{1,3}t your Pollinations account[\s\S]*?service you.{1,3}re using\./gi, '');
  ...
  // ============================================================
  // FINAL CLEAN — remove credit error
  // ============================================================
  finalClean(text) {
    if (!text) return 'No response.';
    let cleaned = text;
