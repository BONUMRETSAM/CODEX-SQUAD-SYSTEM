// ========== ai.js - COMPLETE AI ASSISTANT v35.0.0 ==========
// TRUE FALLBACK: Walang annoying text, walang duplicate, automatic next API
...
module.exports = {
  ...
  description: 'Complete AI assistant with true fallback',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      let response = null;
      try {
        response = await this.callAPI(finalPrompt, prompt, language, intent, topic, subject, requestType);
      } catch (apiError) {
        console.log('[AI] callAPI failed, using fallback...');
        response = await this.callAPIFallback(finalPrompt, prompt, language, intent, topic, subject, requestType);
      }
      ...
      await sendMessage(senderId, {
        text: 'Sorry, hindi available ang AI service ngayon dahil sa technical issue.\n\nSubukan muli mamaya.'
      }, token);
    }
  },

  // ============================================================
  // CALL API — Walang annoying text, automatic next API
  // ============================================================
    ...
    for (const config of apiConfigs) {
      try {
        ...
        try {
          ...
          console.log('[API] ' + config.name + ' — network error, skipping');
          continue;
        }
        ...
        if (errorKeywords.some(kw => dataStr.includes(kw))) {
          console.log('[API] ' + config.name + ' — error detected, skipping');
          continue;
        }

        if (config.successField && config.successValue !== undefined) {
          if (data[config.successField] !== config.successValue) {
            console.log('[API] ' + config.name + ' — invalid field, skipping');
            continue;
          }
        }
        ...
        if (!extracted || typeof extracted !== 'string' || extracted.trim().length === 0) {
          console.log('[API] ' + config.name + ' — empty response, skipping');
          continue;
        }

        const extractedLower = extracted.toLowerCase();
        if (errorKeywords.some(kw => extractedLower.includes(kw))) {
          console.log('[API] ' + config.name + ' — extracted error, skipping');
          continue;
        }
        ...
        // Return agad kapag >= 75 (mabilis at reliable)
        if (quality.isGood && quality.score >= 75) {
          console.log('[API] ' + config.name + ' — GOOD response, returning');
          return formatted;
        }
        ...
        if (quality.score > bestScore) {
          ...
          console.log('[API] ' + config.name + ' — saved as backup (' + quality.score + ')');
        }
      } catch (error) {
        console.log('[API] ' + config.name + ' — failed, skipping');
        continue;
      }
    }
  ...
  // ============================================================
  // FALLBACK API — Last resort kung lahat ng 5 APIs ay failed
  // ============================================================
  async callAPIFallback(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    const fallbackApis = [
      { name: 'DeepAI (retry)', url: 'https://ceddsrestapi.vercel.app/ai/deepai', param: 'message', responsePath: 'result' },
      { name: 'Overchat Qwen (retry)', url: 'https://ceddsrestapi.vercel.app/ai/overchat-qwen', param: 'message', responsePath: 'result', successField: 'operator', successValue: 'Ioarkdev' },
      { name: 'Cedds ChatPlus (retry)', url: 'https://ceddsrestapi.vercel.app/ai/chatplus', param: 'message', responsePath: 'result', successField: 'operator', successValue: 'ceddsdev' },
      { name: 'Opera AI (retry)', url: 'https://betadash-api-swordslush-production.up.railway.app/opera', param: 'ask', responsePath: 'message', successField: 'success' }
    ];

    const errorKeywords = [
      'not enough credits', 'low balance', 'insufficient', 'top up', 'top-up',
      'credits', 'balance', 'quests', 'pollinations.ai', 'api key',
    ...
    for (const config of fallbackApis) {
      try {
        ...
        if (errorKeywords.some(kw => dataStr.includes(kw))) {
          console.log('[Fallback] ' + config.name + ' — error, skipping');
          continue;
        }
        ...
        console.log('[Fallback] ' + config.name + ' — failed');
        continue;
      }
    }
