// ========== ai.js - COMPLETE AI ASSISTANT v33.1.0 ==========
// TRUE FALLBACK: Kahit 1 API lang, may sagot
...
module.exports = {
  ...
  description: 'Complete AI assistant with true fallback guarantee',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (errMsg.includes('credits') || errMsg.includes('balance') || errMsg.includes('top up') || errMsg.includes('all apis failed')) {
        await sendMessage(senderId, { text: 'Sorry, hindi available ang AI service ngayon dahil sa technical issue.\n\nSubukan muli mamaya.' }, token);
        return;
      }
  ...
  // ============================================================
  // CALL API — TRUE FALLBACK (Collect ALL, choose BEST)
  // ============================================================
  async callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    for (const config of apiConfigs) {
      try {
        ...
        if (dataStr.includes('not enough credits') || dataStr.includes('low balance') || dataStr.includes('insufficient') || dataStr.includes('top up')) {
          failedApis.push(config.name + ' (no credits)');
          console.log('[API] ' + config.name + ' — no credits, skipping');
          continue;
        }
        ...
        if (error.response?.data) {
          ...
          if (errData.includes('credits') || errData.includes('balance') || errData.includes('top up')) {
            reason = 'no credits';
          }
        }
    ...
    if (bestResponse) {
      console.log('[API] BEST: ' + bestApiName + ' (score: ' + bestScore + ')');
      return bestResponse;
    }
  ...
  validateResponseQuality(response, intent) {
    ...
    return { isGood: score >= 70, isAcceptable: score >= 40, score: Math.max(0, score) };
  },
