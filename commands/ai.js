// ========== ai.js - COMPLETE AI ASSISTANT v48.0.0 ==========
// STRONG APIs ONLY | High Accuracy | 95% | No Weak APIs
// Priority: Overchat Claude → Overchat DeepSeek → Norch → Chipp
...
// Strong APIs only — walang mahihina
...
module.exports = {
  ...
  description: 'High-accuracy AI assistant - strong APIs only',
  ...
  buildUniversalPrompt(prompt, analysis, previousResponse, previousPrompt) {
    ...
    if (analysis.hasSequence) {
      ...
      finalPrompt += `   - First: preparation (gather, prepare, choose)\n`;
      finalPrompt += `   - Next: main actions (chop, mix, place, layer)\n`;
      finalPrompt += `   - Then: follow-up (cover, sprinkle)\n`;
  ...
  async callCeddsAPI(prompt, url) {
    ...
    // Extract response — different formats
    let extracted = null;
