// ========== ai.js - COMPLETE AI ASSISTANT v30.0.0 ==========
// All-in-one: Greetings, Follow-up commands, Weight, Logic, Research, Math
...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================================
      // STEP 1: GREETINGS & CONVERSATIONAL PHRASES (HIGH PRIORITY)
      // ============================================================
  ...
  isFollowUpCommand(cleanPrompt) {
    const commands = [
      // Elaborate
      'elaborate', 'elaborate more', 'elaborate further', 'explain more', 'explain further',
      'explain in detail', 'explain deeply', 'more explanation', 'add explanation',
      'paliwanag', 'ipaliwanag', 'paliwanagin', 'magpaliwanag', 'paliwanag pa',
      'dagdag paliwanag', 'higit pang paliwanag',

      // Summarize
      'summarize', 'summary', 'sum up', 'sum it up', 'buod', 'i-summarize', 'i-buod',
      'make it short', 'make it brief', 'make it concise', 'shorten', 'shorten it',
      'paikliin', 'paikliin mo', 'iklian', 'iklian mo', 'gawing maikli', 'gawing maiksi',
      'short version', 'brief version', 'concise version', 'maikling bersyon',

      // Paraphrase
      'paraphrase', 'paraphrase it', 'rephrase', 'rephrase it', 'rewrite', 'rewrite it',
      'i-paraphrase', 'i-rephrase', 'i-rewrite', 'baguhin ang salita', 'iba ang salita',
      'ibang salita', 'palitan ang salita', 'bagong salita',

      // Construct
      'construct', 'construct it', 'i-construct', 'rebuild', 'reorganize', 'reorganize it',
      'i-reorganize', 'ayusin', 'i-ayos', 'ayusin ang structure',

      // Simplify
      'simplify', 'simplify it', 'simple', 'make it simple', 'i-simplify', 'pasimplehin',
      'gawing simple', 'gawing madali', 'madaliin', 'simple version', 'easy version',
      'easy to understand', 'madaling maintindihan',

      // Expand
      'expand', 'expand it', 'i-expand', 'expand more', 'dagdagan', 'dagdagan mo',
      'dagdag pa', 'add more', 'add more details', 'more details', 'more info',
      'more information', 'karagdagang impormasyon', 'karagdagang detalye',

      // Translate
      'translate', 'translate it', 'translate to tagalog', 'translate to english',
      'i-translate', 'isalin', 'isalin mo', 'salin', 'ipasalin', 'salihin',
      'isalin sa tagalog', 'isalin sa english', 'i-translate sa tagalog',
      'i-translate sa english',

      // Correct
      'correct', 'correct it', 'fix', 'fix it', 'i-correct', 'i-fix', 'ayusin ang grammar',
      'ayusin ang spelling', 'ayusin ang mali', 'tama', 'itama', 'itama mo',

      // Humanize
      'humanize', 'humanize it', 'i-humanize', 'make it natural', 'make it conversational',
      'gawing natural', 'gawing tao', 'gawing parang tao',

      // Formalize
      'formalize', 'formalize it', 'i-formalize', 'make it formal', 'make it academic',
      'gawing pormal', 'pormalin', 'i-pormal'
    ];

    return commands.includes(cleanPrompt);
  },

  getFollowUpAction(cleanPrompt) {
    if (/summar|buod|short|brief|concise|paikli|iklian|maikli/i.test(cleanPrompt)) return 'summarize';
    if (/paraphrase|rephrase|rewrite|i-rewrite|i-paraphrase|i-rephrase|baguhin|ibang salita/i.test(cleanPrompt)) return 'paraphrase';
    if (/simplify|simple|pasimplehin|gawing simple|madali/i.test(cleanPrompt)) return 'simplify';
    if (/expand|dagdagan|dagdag|add more|more details|karagdagang/i.test(cleanPrompt)) return 'expand';
    if (/translate|isalin|salin|ipasalin/i.test(cleanPrompt)) return 'translate';
    if (/correct|fix|i-correct|i-fix|ayusin|tama|itama/i.test(cleanPrompt)) return 'correct';
    if (/humanize|i-humanize|natural|conversational|gawing natural/i.test(cleanPrompt)) return 'humanize';
    if (/formalize|i-formalize|pormal|academic/i.test(cleanPrompt)) return 'formalize';
    if (/construct|i-construct|rebuild|reorganize|i-reorganize|ayusin ang structure/i.test(cleanPrompt)) return 'construct';
    if (/elaborate|explain more|explain further|paliwanag|ipaliwanag|dagdag paliwanag/i.test(cleanPrompt)) return 'elaborate';
    return 'elaborate';
  },
  ...
  buildFollowUpPrompt(action, previousResponse, userCommand) {
    const actionInstructions = {
      ...
      'summarize': 'SUMMARIZE the following response. Make it SHORT and CONCISE. Keep only the key points. Maximum 200 words.',
      ...
      'humanize': 'HUMANIZE the following response. Make it sound NATURAL and CONVERSATIONAL.',
      'formalize': 'FORMALIZE the following response. Make it ACADEMIC and PROFESSIONAL.',
      'construct': 'CONSTRUCT a new response based on the following. Reorganize into a clear, well-structured format.'
    };
    ...
    prompt += '1. Do NOT explain what "' + action + '" means.\n';
    prompt += '2. Do NOT define the word "' + action + '".\n';
    prompt += '3. Do NOT repeat the title twice.\n';
    prompt += '4. If you keep the TITLE, use the ORIGINAL topic title.\n';
    prompt += '5. Preserve the original meaning and structure.\n';
    prompt += '6. Respond in the SAME language as the original.\n';
  ...
  applyUniversalFormat(response, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    // Skip TITLE for follow-up actions
    const followUpIntents = ['elaborate', 'summarize', 'paraphrase', 'simplify', 'expand', 'translate', 'correct', 'humanize', 'formalize', 'construct', 'followup'];
  ...
  extractTopic(prompt) {
    ...
    // FIX: If topic is just a follow-up command or greeting, return generic
    const followUpOnly = /^(elaborate|summarize|summary|buod|paraphrase|rewrite|rephrase|construct|simplify|expand|shorten|brief|concise|translate|isalin|correct|fix|humanize|formalize|make it short|make it brief|dagdagan)\.?$/i;
