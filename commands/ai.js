// ========== ai.js - COMPLETE AI ASSISTANT v32.0.0 ==========
...
module.exports = {
  ...
  async handleTranslation(senderId, prompt, token) {
    try {
      ...
      finalPrompt += '10. Then give ALL other valid translations. Give 10-20+ if available.\n';
      finalPrompt += '11. Include: idiomatic, formal, casual, slang, regional variants.\n';
      finalPrompt += '12. Do NOT include made-up or unverified translations.\n';
      finalPrompt += '13. NO emojis, NO markdown, NO LaTeX.\n\n';
      ...
      // Remove annoying translation labels
  ...
  async handleWordList(senderId, prompt, token) {
    try {
      ...
      finalPrompt += '8. Aim for 15-30 valid items if possible.\n';
  ...
  buildFollowUpPrompt(action, previousResponse, userCommand) {
    const actionInstructions = {
      ...
      'translate': 'TRANSLATE the following response into TAGALOG. If the phrase is idiomatic, use the IDIOMATIC equivalent, not the literal translation. Output ONLY the translation, nothing else. Do NOT add any label, header, or prefix like "TRANSLATION IN..." or "ALTERNATIVE TRANSLATIONS:". Just write the translation directly.',
  ...
  async callAPI(prompt, originalPrompt, language, intent, topic, subject, requestType) {
    ...
    if (bestResponse) {
      console.log('[API] Best response from ' + bestApiName + ' (score: ' + bestScore + ')');
      return bestResponse;
    }
  ...
  // ============================================================
  // VALIDATE RESPONSE QUALITY — INTENT-SPECIFIC
  // ============================================================
  validateResponseQuality(response, intent) {
    ...
    if (intent === 'definition_process') {
      if (/DEFINITION/i.test(response)) score += 10;
      if (/PROCESS|STEPS|STAGES/i.test(response)) score += 10;
      if (/KEY CHARACTERISTICS/i.test(response)) score += 5;
      if (/EXAMPLES/i.test(response)) score += 5;
      if (/IMPORTANCE/i.test(response)) score += 5;
    }

    if (intent === 'definition') {
      if (response.length >= 100 && response.length <= 800) score += 15;
      if (/is\s+a|is\s+the|refers\s+to|means/i.test(response)) score += 10;
    }

    if (intent === 'wordlist') {
      const items = response.match(/^\d+\./gm);
      if (items && items.length >= 15) score += 20;
      else if (items && items.length >= 10) score += 10;
      else if (items && items.length >= 5) score += 5;
      else score -= 20;
    }
  ...
  detectSubject(prompt) {
    ...
    if (/(define|explain|describe)\s+.+\s+(and|at)\s+(its|the)\s+(process|processes|steps|stages|cycle)/i.test(lower)) return 'definition_process';
  ...
  detectIntent(prompt) {
    ...
    // COMBINED: definition + processes
    if (/^(define|explain|describe|discuss)\s+.+\s+(and|at)\s+(its|the|ang)\s+(process|processes|steps|stages|cycle|phases|procedure)/i.test(lower)) return 'definition_process';
    if (/^(what is|what are|ano ang)\s+.+\s+(and|at)\s+(its|the|ang)\s+(process|processes|steps|stages|cycle|phases|procedure)/i.test(lower)) return 'definition_process';
  ...
  detectRequestType(prompt) {
    ...
    if (/(define|explain|describe)\s+.+\s+(and|at)\s+(its|the)\s+(process|processes|steps|stages|cycle)/i.test(lower)) return 'detailed';
  ...
  buildDefinitionProcessPrompt(prompt, langName) {
    let finalPrompt = '';
    finalPrompt += 'You are a helpful AI assistant. Give a COMPLETE but CONCISE answer that covers BOTH the definition AND the processes.\n\n';
    finalPrompt += 'CRITICAL RULES:\n';
    ...
    finalPrompt += '5. Be ACCURATE and PRECISE. Do NOT hallucinate.\n';
    ...
    finalPrompt += '[2-4 sentences definition]\n\n';
    finalPrompt += 'II. KEY CHARACTERISTICS\n';
    finalPrompt += '1. [characteristic]\n';
    finalPrompt += '2. [characteristic]\n';
    finalPrompt += '3. [characteristic]\n';
    finalPrompt += '4. [characteristic]\n\n';
    finalPrompt += 'III. PROCESSES / STEPS\n';
    finalPrompt += '1. [process/step]\n';
    finalPrompt += '2. [process/step]\n';
    finalPrompt += '3. [process/step]\n';
    finalPrompt += '4. [process/step]\n';
    finalPrompt += '5. [process/step]\n\n';
    finalPrompt += 'IV. EXAMPLES\n';
    finalPrompt += '1. [example]\n';
    finalPrompt += '2. [example]\n';
    finalPrompt += '3. [example]\n\n';
    finalPrompt += 'V. IMPORTANCE\n';
    finalPrompt += '1. [importance]\n';
    finalPrompt += '2. [importance]\n';
    finalPrompt += '3. [importance]\n\n';
  ...
  buildBriefPrompt(prompt, langName, intent) {
    ...
    finalPrompt += '6. Do NOT hallucinate. Be ACCURATE and PRECISE.\n';
    finalPrompt += '7. NO emojis, NO markdown, NO LaTeX.\n';
    finalPrompt += '8. Respond in ' + langName.toUpperCase() + ' language.\n\n';
  ...
  buildLogicPrompt(prompt, langName) {
    ...
    finalPrompt += '8. Respond in ' + langName.toUpperCase() + ' language.\n';
    finalPrompt += '9. Keep it CONCISE and ACCURATE.\n';
  ...
  buildWordListPrompt(prompt, langName) {
    return 'You are a dictionary expert. Give ALL common synonyms/terms for the word in the question.\n\n' +
      ...
      '6. Respond in ' + langName.toUpperCase() + ' language.\n' +
  ...
  buildTranslationPrompt(prompt, langName) {
    return 'You are a professional TRANSLATOR fluent in ALL languages.\n\n' +
      ...
      '8. Give 10-20+ translations if available.\n' +
