// ========== ai.js - FINAL v57.0.0 ==========
// COMPLETE | No Removed Functions | No Bugs | No Errors | No Issues
// All 60+ Functions | Math Fix | Unit Conversion | Translate Fix
...
module.exports = {
  ...
  description: 'Complete AI assistant - all functions, no bugs',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (isFollowUpCommand) {
        if (!previousResponse) {
          // Walang previous response — ituring as new question
          console.log('[AI] Follow-up but no context — treating as new question');
        } else {
  ...
  comprehendInput(prompt) {
    ...
    analysis.hasStepByStep = /solve\s+step\s+by\s+step|show\s+solution|full\s+solution|complete\s+solution|with\s+solution|with\s+step|solve\s+and\s+solution|answer\s+and\s+solve|solve\s+this|solve\s+the\s+following/i.test(lower);
    ...
    // MATH (fixed — walang \b boundary)
    const hasMathKeywords = /(solve|compute|calculate|evaluate|find\s+x|find\s+the\s+value|equation|solution|derivative|integral|limit|what\s+is\s+the\s+value\s+of|answer\s+and\s+solve|solve\s+this)/i.test(lower);
    const hasPureNumbers = /^[\d\s\+\-\*\/\(\)\.\,×÷\^]+$/i.test(prompt.replace(/\s/g, ''));
    const hasMathEquations = /\d+\s*[\+\-\*\/\^]\s*\d+|\d*x\s*[\+\-\*\/]\s*\d+|x\s*[\+\-\*\/]\s*\d+|\d+\s*=\s*\d+/i.test(prompt);
    
    if (hasMathKeywords || hasPureNumbers || hasMathEquations || analysis.hasStepByStep) {
    ...
    if (analysis.hasNumbers || analysis.hasParts || analysis.hasABCD) {
      if (analysis.hasInstructions) {
        ...
        else if (analysis.type === 'general') analysis.type = 'quiz';
      }
    }
  ...
  buildUniversalPrompt(prompt, analysis, previousResponse, previousPrompt) {
    ...
    if (analysis.hasSequence) {
      ...
      finalPrompt += `2. Identify the STARTING step: Usually "Gather", "Prepare", "Get", "Collect".\n`;
      finalPrompt += `3. Identify the ENDING step: Usually "Wait", "Finish", "Complete", "Harvest", "Use", "Filter".\n`;
  ...
  // ============================================================
  // FOLLOW-UP — COMPLETE (all functions, no removed)
  // ============================================================
  isFollowUpCommand(cleanPrompt) {
    ...
    // UNIT CONVERSION
    if (/^\d+\.?\d*\s*(lbs?|pounds?|kg|kilograms?|g|grams?|oz|ounces?|cm|centimeters?|m|meters?|ft|feet|in|inches?|km|kilometers?|mi|miles?|c|celsius|f|fahrenheit|k|kelvin)\s*(to|sa|into|->)\s*(lbs?|pounds?|kg|kilograms?|g|grams?|oz|ounces?|cm|centimeters?|m|meters?|ft|feet|in|inches?|km|kilometers?|mi|miles?|c|celsius|f|fahrenheit|k|kelvin)/i.test(p)) return true;
  ...
  buildFollowUpPrompt(action, previousResponse, userPrompt) {
    const actionInstructions = {
      'elaborate': 'ELABORATE. Add MORE DETAILS, more explanation, and deeper analysis.',
      'purpose': 'PURPOSE. Explain the PURPOSE of this content. Why is this important? What is it for?',
      'examples': 'GIVE EXAMPLES. Provide SPECIFIC, CONCRETE examples related to the content.',
      'math_solution': 'FULL SOLUTION. Show the COMPLETE step-by-step solution. Include formulas, equations, and final answer.',
      'convert': 'CONVERT. Convert the measurement. Show the conversion steps and result.',
