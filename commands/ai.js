...
module.exports = {
  ...
  description: 'Complete AI assistant with strict math detection',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const finalPrompt = this.buildPrompt(prompt, previousResponse, previousPrompt, requestType);
      ...
      aiResponse = this.formatResponse(aiResponse, requestType);
  ...
  // ============================================================
  // SPLIT INTELLIGENTLY - Hindi mapuputol ang sentences
  // ============================================================
  ...
  detectRequestType(prompt) {
    ...
    if (/explain|paliwanag|ipaliwanag|detail|elaborate/i.test(lower)) {
      return 'explain';
    }
    return 'general';
  },
  ...
  // ============================================================
  formatResponse(response, requestType) {
  ...
  // ============================================================
  // STRICT MATH DETECTION - Only compute if PURE math expression
  // ============================================================
  tryMathCompute(prompt) {
    ...
    // ===== REJECT IF MAY NON-MATH KEYWORDS =====
    const nonMathKeywords = [
      ...
      'year', 'taon', 'month', 'buwan', 'day', 'araw',
      'time', 'oras', 'date', 'petsa',
      'name', 'pangalan', 'address', 'tirahan',
      'number', 'numero', 'phone', 'telepono'
    ];
    ...
    // ===== MUST BE PURE MATH EXPRESSION =====
    if (!/^[\d\s\+\-\*\/\(\)\.\,×÷]+$/.test(prompt.replace(/\s/g, ''))) return null;

    // ===== REMOVE COMMAS AND TRAILING OPERATORS =====
  ...
  // BUILD PROMPT
  // ============================================================
  buildPrompt(prompt, previousResponse, previousPrompt, requestType) {
    let finalPrompt = '';
    ...
    finalPrompt += `4. Do NOT treat non-math questions as math.\n`;
    finalPrompt += `5. Use plain text only.\n`;
    ...
    if (requestType === 'summarize') {
      ...
      finalPrompt += `- Give detailed explanation.\n`;
    } else {
