...
// COMPREHENSION-BASED | Universal | No Hardcoded Answers/Formats
// Mirrors user input EXACTLY - nothing added, nothing removed
...
module.exports = {
  ...
  description: 'Universal AI assistant - comprehension-based, mirrors user input',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================================
      // STEP 3: WORD LIST (special handling lang ito)
      // ============================================================
      ...
      // ============================================================
      // STEP 4: WEIGHT ESTIMATION (may sariling formula)
      // ============================================================
      ...
      // ============================================================
      // STEP 5: REGULAR AI (lahat ng iba)
      // ============================================================
      if (!prompt && !isReply) {
      ...
      // ============================================================
      // STEP 6: COMPREHEND (analyze ang user input)
      // ============================================================
      const analysis = this.comprehendInput(prompt);
      
      console.log('[AI] Type:', analysis.type, '| HasNumbers:', analysis.hasNumbers, '| Instructions:', analysis.instructions.join(', '), '| Length:', prompt.length);

      // ============================================================
      // STEP 7: BUILD UNIVERSAL PROMPT (dynamic, base sa input)
      // ============================================================
      const finalPrompt = this.buildUniversalPrompt(prompt, analysis, previousResponse, previousPrompt);

      // ============================================================
      // STEP 8: CALL API (Priority-based)
      // ============================================================
      ...
      // ============================================================
      // STEP 10: LIGHT CLEAN (hindi aggressive)
      // ============================================================
  ...
  // ============================================================
  // COMPREHEND INPUT (dynamic analysis - walang hardcoded answers)
  // ============================================================
  comprehendInput(prompt) {
    if (!prompt) return { type: 'general', hasNumbers: false, instructions: [], language: 'english' };
    ...
    // Detect instructions (Panuto, Directions, etc.)
    analysis.hasInstructions = /panuto|directions|instructions|piliin|isulat|sagutin|choose|write|answer|explain|solve|enumerate|fill|match|compute|calculate|list|define|describe|discuss/i.test(lower);

    // Collect specific instructions
    if (/\bsolve\b|\bcompute\b|\bcalculate\b/i.test(lower)) analysis.instructions.push('solve');
    if (/\bexplain\b|\bipaliwanag\b/i.test(lower)) analysis.instructions.push('explain');
    if (/\belaborate\b|\bexpand\b|\bmore\s+details\b/i.test(lower)) analysis.instructions.push('elaborate');
    if (/\bgive\s+examples?\b|\bprovide\s+examples?\b|\bhalimbawa\b/i.test(lower)) analysis.instructions.push('examples');
    if (/\bcompare\b|\bdifference\b|\bsimilarit/i.test(lower)) analysis.instructions.push('compare');
    if (/\bprocess\b|\bstages?\b|\bcycle\b|\bphases?\b/i.test(lower)) analysis.instructions.push('process');
    if (/\breasoning\b|\breason\b|\bwhy\b|\bbakit\b/i.test(lower)) analysis.instructions.push('reasoning');
    if (/\bessay\b|\bsanaysay\b|\bwrite\s+about\b/i.test(lower)) analysis.instructions.push('essay');
    if (/\bdefine\b|\bdefinition\b|\bmeaning\b|\bkahulugan\b/i.test(lower)) analysis.instructions.push('define');
    if (/\blist\b|\benumerate\b|\bisa-isahin\b/i.test(lower)) analysis.instructions.push('list');
    if (/\btrue\s+or\s+false\b|\btama\s+o\s+mali\b/i.test(lower)) analysis.instructions.push('truefalse');
    if (/\bmatch\b|\bitapat\b|\btugmain\b/i.test(lower)) analysis.instructions.push('match');
    if (/\bfill\s+in\s+the\s+blank\b|\bpunan\s+ang\s+patlang\b/i.test(lower)) analysis.instructions.push('fillblank');
    ...
    if (
      ...
      /\bcalculate\b|\bcompute\b|\bsimplify\b|\bevaluate\b|\bderivative\b|\bintegral\b|\bfactor\b|\bexpand\b/i.test(lower) ||
      /\bsin\b|\bcos\b|\btan\b|\blog\b|\bln\b|\bsqrt\b|\bsquare\s+root\b/i.test(lower) ||
    ...
    // Detect logic
    if (
      /\blogic\b|\breasoning\b|\bpattern\b/i.test(lower) ||
      /\briddle\b|\bbugtong\b|\bpuzzle\b/i.test(lower) ||
      ...
      /\bwhat\s+comes\s+next\b/i.test(lower)
    ) {
      analysis.isLogic = true;
      if (!analysis.isMath) analysis.type = 'logic';
    }

    // Detect quiz/exam/activity sheet
    if (analysis.hasNumbers || analysis.hasABCD || analysis.hasParts) {
      if (analysis.hasInstructions) {
        if (/activity\s+sheet|worksheet|gawain/i.test(lower)) {
          analysis.isQuiz = true;
      ...
      } else if (analysis.hasNumbers || analysis.hasABCD) {
        analysis.isQuiz = true;
        analysis.type = 'quiz';
      }
    }
    ...
    // Detect coding
    if (/\bpython\b|\bjava\b|\bjavascript\b|\bc\+\+\b|\bprint\s*\(|\bconsole\.log|\bfunction\s+\w+\s*\(|\bcode\b/i.test(lower)) {
      analysis.isCoding = true;
      analysis.type = 'coding';
    }

    // Detect essay
    if (/\b(essay|sanaysay|write\s+an?\s+essay|composition|magsulat\s+ng)\b/i.test(lower)) {
      analysis.isEssay = true;
      analysis.type = 'essay';
    }
    ...
    // Detect compare
    if (/\bcompare\b|\bdifference\s+between\b|\bsimilarities\b|\bvs\.?\b|\bversus\b|\bcontrast\b/i.test(lower)) {
      analysis.isCompare = true;
      if (analysis.type === 'general') analysis.type = 'compare';
    }

    // Detect process
    if (/\bprocess\s+of\b|\bstages\s+of\b|\bcycle\s+of\b|\bphases\s+of\b|\bprocedure\b/i.test(lower)) {
      analysis.isProcess = true;
      if (analysis.type === 'general') analysis.type = 'process';
    }
  ...
  // ============================================================
  // BUILD UNIVERSAL PROMPT (dynamic - base lang sa input)
  // ============================================================
  buildUniversalPrompt(prompt, analysis, previousResponse, previousPrompt) {
    ...
    // ============================================================
    // CORE RULES (universal - walang hardcoded format)
    // ============================================================
    finalPrompt += `=== YOUR TASK ===\n`;
    finalPrompt += `Read the input above carefully. Understand what is being asked. Answer completely and accurately.\n\n`;

    finalPrompt += `=== ABSOLUTE RULES ===\n`;
    finalPrompt += `1. FOLLOW the instructions in the input EXACTLY.\n`;
    finalPrompt += `2. MIRROR the format of the input. If input has "1.", answer "1.". If "A.", answer "A.". If "Part I", keep "Part I".\n`;
    finalPrompt += `3. Answer ONLY what is asked. NOTHING ADDED, NOTHING REMOVED.\n`;
    finalPrompt += `4. Do NOT add "Answer:", "Final Answer:", "Solution:", "Reasoning:" labels unless the input has them.\n`;
    finalPrompt += `5. Do NOT add intro ("Here is", "Let me") or outro ("Hope this helps").\n`;
    finalPrompt += `6. Do NOT add sections ("I. DEFINITION", "II. KEY POINTS") unless the input has them.\n`;
    ...
    // ============================================================
    // INSTRUCTION-SPECIFIC GUIDANCE (dynamic)
    // ============================================================
    if (analysis.instructions.length > 0) {
      finalPrompt += `=== SPECIFIC INSTRUCTIONS DETECTED ===\n`;
      for (const instr of analysis.instructions) {
        switch (instr) {
          case 'solve':
            finalPrompt += `- SOLVE: Show step-by-step solution. For equations with result = 1 or specific values, consider ALL cases (e.g., base = 1, base = -1 with even exponent, exponent = 0 with base ≠ 0). Verify each solution.\n`;
            break;
          case 'explain':
            finalPrompt += `- EXPLAIN: Provide clear, complete explanation with reasons.\n`;
            break;
          case 'elaborate':
            finalPrompt += `- ELABORATE: Provide more details, examples, and deeper explanation.\n`;
            break;
          case 'examples':
            finalPrompt += `- GIVE EXAMPLES: Provide specific concrete examples.\n`;
            break;
          case 'compare':
            finalPrompt += `- COMPARE: Show similarities and differences clearly.\n`;
            break;
          case 'process':
            finalPrompt += `- PROCESS: List all stages or steps in order.\n`;
            break;
          case 'reasoning':
            finalPrompt += `- REASONING: Show the reasoning clearly. Explain why.\n`;
            break;
          case 'essay':
            finalPrompt += `- ESSAY: Write a complete essay with introduction, body, and conclusion.\n`;
            break;
          case 'define':
            finalPrompt += `- DEFINE: Give a clear definition with key characteristics.\n`;
            break;
          case 'list':
            finalPrompt += `- LIST: Provide a complete list. Number each item.\n`;
            break;
          case 'truefalse':
            finalPrompt += `- TRUE/FALSE: Write TRUE/FALSE (or TAMA/MALI) for each item.\n`;
            break;
          case 'match':
            finalPrompt += `- MATCHING: Match items from Column A to Column B.\n`;
            break;
          case 'fillblank':
            finalPrompt += `- FILL IN BLANK: Provide the correct word/phrase for each blank.\n`;
            break;
        }
      }
      finalPrompt += `\n`;
    ...
    // ============================================================
    // FORMAT GUIDANCE (dynamic based sa hasNumbers)
    // ============================================================
    if (!analysis.hasNumbers) {
  ...
  // ============================================================
  // CALL API - PRIORITY BASED (First success wins)
  // ============================================================
  ...
  // ============================================================
  // CLEAN OUTPUT (LIGHT - hindi aggressive)
  // ============================================================
  cleanOutput(text) {
    if (!text) return '';
    let cleaned = text.trim();

    // Remove meta-info lang (kung meron)
    ...
    // Remove common AI artifacts (kung meron)
  ...
  async verifyCompleteness(response, originalPrompt) {
    ...
    retryPrompt += `The following items are MISSING or BLANK. Answer them:\n\n`;
    retryPrompt += missingItems.join('\n\n') + '\n\n';
    retryPrompt += `=== RULES ===\n`;
    retryPrompt += `- MIRROR the same format as the original.\n`;
