// ========== ai.js - FINAL COMPLETE v58.0.0 ==========
// ALL 60+ FUNCTIONS | Groq + Cerebras + Overchat | No Removed | No Bugs
...
module.exports = {
  ...
  description: 'Complete AI assistant - all 60+ functions',
  usage: 'ai [question]',
  version: '58.0.0',
  author: 'codex',
  category: 'AI',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (this.isRepeat(aiResponse, prompt)) {
        ...
        aiResponse = await this.callAIWithViceVersa(finalPrompt + '\n\nIMPORTANT: Do NOT repeat the input. Give the ANSWER only.', prompt, 'english', analysis.type);
      }
  ...
  buildStrictPrompt(prompt, analysis) {
    ...
    finalPrompt += `3. Do NOT add intro or outro.\n`;
    finalPrompt += `4. Do NOT add "Would you like..." or any follow-up.\n`;
    finalPrompt += `5. Output ONLY the ANSWER.\n\n`;
    finalPrompt += `=== YOUR TASK ===\nAnswer the input above. MIRROR the format. NO blanks. NO repeats.\n\n`;

    if (analysis.type === 'math') {
      finalPrompt += `=== FOR MATH (SOLVE STEP BY STEP) ===\n`;
  ...
  tryUnitConversion(prompt) {
    ...
    const match = lower.match(/^(\d+\.?\d*)\s*(lbs?|pounds?|kg|kilograms?|g|grams?|oz|ounces?|cm|centimeters?|m|meters?|ft|feet|in|inches?|km|kilometers?|mi|miles?|c|celsius|f|fahrenheit|k|kelvin)\s*(?:to|sa|into|->)\s*(lbs?|pounds?|kg|kilograms?|g|grams?|oz|ounces?|cm|centimeters?|m|meters?|ft|feet|in|inches?|km|kilometers?|mi|miles?|c|celsius|f|fahrenheit|k|kelvin)/i);
    if (!match) return null;
    const value = parseFloat(match[1]);
    const from = this.normalizeUnit(match[2]);
    const to = this.normalizeUnit(match[3]);
    const conversions = {
      'lbs->kg': value * 0.453592, 'kg->lbs': value * 2.20462, 'kg->g': value * 1000, 'g->kg': value / 1000,
      'oz->g': value * 28.3495, 'g->oz': value / 28.3495, 'cm->m': value / 100, 'm->cm': value * 100,
      'm->ft': value * 3.28084, 'ft->m': value / 3.28084, 'cm->in': value / 2.54, 'in->cm': value * 2.54,
      'km->mi': value * 0.621371, 'mi->km': value / 0.621371, 'm->km': value / 1000, 'km->m': value * 1000,
      'ft->in': value * 12, 'in->ft': value / 12, 'c->f': (value * 9/5) + 32, 'f->c': (value - 32) * 5/9,
      'c->k': value + 273.15, 'k->c': value - 273.15, 'lbs->g': value * 453.592, 'g->lbs': value / 453.592
    };
    const key = from + '->' + to;
  ...
  comprehendInput(prompt) {
    ...
    if (hasMathKeywords || hasPureNumbers || hasMathEquations) { analysis.type = 'math'; return analysis; }

    if (/\bif\s+x\s+and\s+y\b/i.test(lower) || /\breal\s+numbers\b/i.test(lower) || /\blogic\b|\breasoning\b|\briddle\b|\bpuzzle\b|\bbugtong\b/i.test(lower)) { analysis.type = 'logic'; return analysis; }

    if (/\bpython\b|\bjava\b|\bjavascript\b|\bprint\s*\(|\bconsole\.log/i.test(lower)) { analysis.type = 'coding'; return analysis; }

    if (/(^|\n)\s*\d+\s*[\.\)]\s*\S/i.test(prompt) && /panuto|directions|choose|write|answer/i.test(lower)) {
      if (/activity\s+sheet|worksheet/i.test(lower)) analysis.type = 'activity_sheet';
      else if (/exam|test|quiz/i.test(lower)) analysis.type = 'exam';
  ...
  async callAIWithViceVersa(prompt, originalPrompt, language, intent) {
    ...
    for (const provider of providers) {
      try {
        ...
        return result.trim();
      } catch (error) { console.log(`[AI] ✗ ${provider.name}: ${error.message}`); }
    }
    throw new Error('All AI providers failed');
  },

  hasErrorKeyword(lower) {
    const keywords = ['not enough credits', 'insufficient', 'quota', 'exceeded', 'limit reached', 'rate limit', 'too many requests', 'api key', 'out of credits'];
  ...
  async callGroqAPI(prompt) {
    ...
    const r = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
      ...
      messages: [
        { role: 'system', content: 'You are an accurate AI assistant. NEVER repeat the input question. Give the ANSWER directly. For math: show step-by-step. For activity sheets: mirror format.' },
  ...
  async callCerebrasAPI(prompt) {
    ...
    const r = await axios.post('https://api.cerebras.ai/v1/chat/completions', {
      ...
      temperature: 0.3, max_tokens: 8000
    }, { headers: { 'Authorization': `Bearer ${CEREBRAS_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 90000 });
    return r.data?.choices?.[0]?.message?.content;
  },

  async callCeddsAPI(prompt, url) {
    const r = await axios.get(`${url}?message=${encodeURIComponent(prompt)}`, { timeout: 90000, headers: { 'Accept': 'application/json' } });
    const data = r.data;
  ...
  async sendComplete(senderId, text, token) {
    if (!text) return;
    if (text.length <= MAX_CHUNK) { await sendMessage(senderId, { text }, token); return; }
  ...
  isGreetingOrConversational(prompt) {
    ...
    const patterns = [
      ...
      /^(magandang\s*(araw|umaga|hapon|gabi))$/i,
      /^(help|tulong)$/i, /^(thank|thanks|salamat)$/i, /^(ok|okay|sige|sure|noted)$/i,
      /^(bye|goodbye|paalam|ingat)$/i, /^(sorry|pasensya)$/i,
      /^(yes|oo|opo|no|hindi)$/i, /^(good|nice|great|galing|magaling)$/i,
      /^(sino|who)\s*(ka|ka po)$/i, /^(ano|what)\s*(pangalan mo|name mo)$/i
    ];
  ...
  buildConversationalReply(prompt) {
    ...
    if (/^(help|tulong)$/i.test(p)) return 'HELP GUIDE\n\n1. AI - Magtanong ng kahit ano\n2. WEIGHT - Estimate ng timbang\n3. UNIT CONVERSION - "84.4 lbs to kg"\n4. ACTIVITY SHEET - I-paste lang\n\nFOLLOW-UP: elaborate, summarize, translate, cite, quiz, flashcards, at iba pa';
    ...
    if (/^(sorry|pasensya)$/i.test(p)) return 'Walang problema!';
    if (/^(yes|oo|opo)/i.test(p)) return 'Sige!';
    if (/^(no|hindi)/i.test(p)) return 'Okay.';
    if (/^(good|nice|great|galing|magaling)$/i.test(p)) return 'Salamat!';
    if (/^(sino|who)\s*(ka|ka po)$/i.test(p)) return 'Ako si Teacher Arlene.';
  ...
  buildFollowUpPrompt(action, previousResponse, userPrompt) {
    const actionInstructions = {
      ...
      'convert': 'CONVERT. Convert the measurement. Show the steps.',
  ...
  async handleWeightEstimation(senderId, prompt, token) {
    ...
    if (!girth) { await sendMessage(senderId, { text: 'Please provide heart girth and length.' }, token); return; }
    ...
    if (!length) { await sendMessage(senderId, { text: 'Please provide both girth and length.' }, token); return; }
