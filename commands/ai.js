// ========== ai.js - COMPLETE AI ASSISTANT v45.0.0 ==========
// Sequence reasoning | ✓/✗ labels | Arrows | No duplicates | Organized
...
module.exports = {
  ...
  description: 'Universal AI assistant - sequence reasoning, clean output',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      console.log('[AI] Type:', analysis.type, '| HasNumbers:', analysis.hasNumbers, '| HasSequence:', analysis.hasSequence, '| HasCheckX:', analysis.hasCheckX, '| Length:', prompt.length);
  ...
  comprehendInput(prompt) {
    ...
    // DETECT ✓/✗
    // ============================================================
    analysis.hasCheckX = /[✓✗]|proper.*improper|improper.*proper|check.*x\s+if|write.*[✓✗]/i.test(prompt) ||
                        (/proper/i.test(lower) && /improper/i.test(lower));
  ...
  // ============================================================
  // BUILD UNIVERSAL PROMPT (with sequence + ✓/✗ specific rules)
  // ============================================================
  buildUniversalPrompt(prompt, analysis, previousResponse, previousPrompt) {
    ...
    if (analysis.hasSequence) {
      ...
      finalPrompt += `   - Usually begins with "Gather", "Prepare", "Get", "Collect", "Choose", "Mix"\n`;
      finalPrompt += `   - The FIRST action that must happen before anything else\n`;
      finalPrompt += `3. Identify the ENDING step:\n`;
      finalPrompt += `   - Usually begins with "Wait", "Finish", "Complete", "Harvest", "Use", "Filter"\n`;
      finalPrompt += `   - The LAST action after everything is done\n`;
      finalPrompt += `4. For EACH step, ask yourself:\n`;
      finalPrompt += `   - "What MUST happen BEFORE this step?"\n`;
      finalPrompt += `   - "What MUST happen AFTER this step?"\n`;
      finalPrompt += `5. Arrange in logical order:\n`;
      finalPrompt += `   - First: preparation/setup (gather, prepare, choose)\n`;
      finalPrompt += `   - Next: main actions (place, layer, mix, chop)\n`;
      finalPrompt += `   - Then: follow-up actions (cover, sprinkle)\n`;
      finalPrompt += `   - Last: waiting/finishing (wait, filter, use)\n\n`;
      finalPrompt += `EXAMPLE (Basket Composting):\n`;
      finalPrompt += `Given steps:\n`;
      finalPrompt += `- Cover the basket with soil\n`;
      finalPrompt += `- Gather materials\n`;
      finalPrompt += `- Place the basket in a hole\n`;
      finalPrompt += `- Layer the materials\n`;
      finalPrompt += `- Wait for decomposition\n`;
      finalPrompt += `- Sprinkle water\n\n`;
      finalPrompt += `Reasoning:\n`;
      finalPrompt += `1. Gather materials → FIRST (need materials first)\n`;
      finalPrompt += `2. Place the basket in hole → SECOND (setup)\n`;
      finalPrompt += `3. Layer the materials → THIRD (main action)\n`;
      finalPrompt += `4. Cover with soil → FOURTH (after layering)\n`;
      finalPrompt += `5. Sprinkle water → FIFTH (keep moist)\n`;
      finalPrompt += `6. Wait for decomposition → LAST (finishing)\n\n`;
      finalPrompt += `OUTPUT FORMAT:\n`;
      finalPrompt += `- KEEP the ORIGINAL item order (do NOT reorder the items in the output)\n`;
      finalPrompt += `- Write the CORRECT NUMBER next to each item\n`;
      finalPrompt += `- Format: "[correct number] [original text]"\n`;
      finalPrompt += `- Example:\n`;
      finalPrompt += `  1 Gather biodegradable materials such as vegetable peelings and dried leaves.\n`;
      finalPrompt += `  2 Place the basket in a shallow hole in the garden.\n`;
      finalPrompt += `  3 Layer the green and dry materials inside the basket.\n`;
      finalPrompt += `  4 Cover the basket with soil or dried leaves.\n`;
      finalPrompt += `  5 Sprinkle water to keep the compost moist.\n`;
      finalPrompt += `  6 Wait for the materials to decompose into compost.\n\n`;
      finalPrompt += `DISCREPANCY CHECK:\n`;
      finalPrompt += `- Count the number of items.\n`;
      finalPrompt += `- If instructions say "1-5" but there are 6 items, note: "Note: 6 items found, but instruction says 1-5."\n\n`;
    }
    ...
    if (analysis.hasCheckX) {
      ...
      finalPrompt += `- For EACH statement, think:\n`;
      finalPrompt += `  - Is this a CORRECT/GOOD practice? → ✓ (Proper)\n`;
      finalPrompt += `  - Is this a WRONG/BAD practice? → ✗ (Improper)\n`;
      finalPrompt += `- OUTPUT FORMAT: "[number]. [✓ or ✗] [Proper/Improper]"\n`;
      finalPrompt += `- Example:\n`;
      finalPrompt += `  1. ✓ Proper\n`;
      finalPrompt += `  2. ✗ Improper\n`;
      finalPrompt += `  3. ✓ Proper\n\n`;
      finalPrompt += `IMPORTANT: Every number MUST have either ✓ or ✗. NO BLANKS.\n\n`;
    }
    ...
    if (analysis.hasTrueFalse) {
      ...
      finalPrompt += `- Example:\n`;
      finalPrompt += `  1. TRUE - The sun rises in the east.\n`;
      finalPrompt += `  2. FALSE - The sun rises in the west.\n\n`;
    }
    ...
    if (analysis.hasMatching) {
      ...
      finalPrompt += `- Example:\n`;
      finalPrompt += `  1. B - Mammal\n`;
      finalPrompt += `  2. C - Flower\n\n`;
    }
  ...
  // ============================================================
  // FOLLOW-UP (with summarize, humanize, translate)
  // ============================================================
  isFollowUpCommand(cleanPrompt) {
    ...
    if (/^(summarize|summary|buod|paikliin|shorten|brief|make it short|make it brief)( it| this| that| mo| po| lang)?$/i.test(p)) return true;
    if (/^(humanize|make it human|make it natural|make it sound human|gawing natural|gawing tao)( it| this| that| mo| po| lang)?$/i.test(p)) return true;
  ...
  buildFollowUpPrompt(action, previousResponse) {
    const actionInstructions = {
      ...
      'humanize': 'HUMANIZE. Rewrite to sound more NATURAL and HUMAN. Avoid robotic AI tone. Use conversational but clear language.',
      'translate': 'TRANSLATE. Translate the text to the requested language. Keep the same meaning.'
    };
