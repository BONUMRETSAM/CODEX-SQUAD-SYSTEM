// ========== gemini.js - UNIVERSAL SOLUTION PROVIDER ==========
const axios = require('axios');
const { sendMessage } = require('../handles/sendMessage');

module.exports = {
  name: ['gemini'],
  description: 'Universal image analyzer with solution support for all subjects',
  ...
  buildGuidelinesPrompt(ocrText, userPrompt, language) {
    ...
    let prompt = `You are analyzing GUIDELINES or INSTRUCTIONS.\n\n`;
    prompt += `GUIDELINES FROM IMAGE:\n${text}\n\n`;
    prompt += `TASK:\n`;
    prompt += `Do NOT just copy the guidelines. Instead, APPLY the guidelines by creating a SAMPLE OUTPUT.\n\n`;
    prompt += `REQUIREMENTS:\n`;
    prompt += `1. Follow each step in the guidelines.\n`;
    prompt += `2. Create a SPECIFIC EXAMPLE for each step.\n`;
    prompt += `3. Use a SAMPLE TOPIC related to the guidelines.\n`;
    prompt += `4. Show the complete output for each step.\n`;
    prompt += `5. Include examples for each item.\n`;
    prompt += `6. End with a Final Answer summary.\n\n`;
    prompt += `FORMAT:\n`;
    prompt += `Answer:\n`;
    prompt += `[Brief intro about the guidelines]\n\n`;
    prompt += `TOPIC:\n[Sample topic]\n\n`;
    prompt += `OBJECTIVES:\n[Objectives]\n\n`;
