...
module.exports = {
  name: ['gemini'],
  description: 'Universal image analyzer using Norch API',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== STEP 4: Call Norch API ONLY =====
      let cleanResponse = await this.callNorchAPI(prompt, imageUrl);

      if (!cleanResponse || cleanResponse.length < 5) {
        // Retry with simpler prompt
        console.log('[Retry] Trying simpler prompt...');
        const simplePrompt = `Analyze this image and answer all questions. Respond in ${this.getLanguageName(imageLanguage)}.`;
        cleanResponse = await this.callNorchAPI(simplePrompt, imageUrl);
      }
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (finalPrompt.length > 4000) {
          ...
          console.log('[Norch] Prompt truncated to 4000 chars');
        }
        ...
        if (attempts < maxAttempts) {
          ...
          console.log(`[Norch] Waiting ${delay}ms before retry...`);
          await new Promise(r => setTimeout(r, delay));
        }
      }
    }

    console.log('[Norch] All attempts failed:', lastError?.message);
    return '';
  },
